...
const t = (secs: number) =>
  new Date(1_700_000_000_000 + secs * 1000).toISOString();
...
describe("toRatePerSecond", () => {
  ...
  test("single point uses fallback step and negatives floor at zero", () => {
    expect(toRatePerSecond([{ time: t(0), value: 300 }], 30)[0]?.value).toBe(
      10,
    );
...
describe("parseNodeId", () => {
  test("accepts whole non-negative integers only", () => {
    ...
    expect(parseNodeId("0")).toBe(0);
    expect(parseNodeId(" 12 ")).toBe(12);
    for (const bad of ["1abc", "1.5", "-1", "", "abc", "0x10"]) {
      expect(() => parseNodeId(bad)).toThrow(/Invalid --node/);
    }
  });
});
