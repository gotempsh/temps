...
import { newline, header, json, colors, keyValue, info } from '../../ui/output.js'
...
export function toRatePerSecond(
  ...
  const stepFor = (i: number): number => {
    ...
    const gap = (i > 0 && prev != null ? here - prev : (next ?? NaN) - here) / 1000
    return Number.isFinite(gap) && gap > 0 ? gap : fallbackStepSeconds
  }
  return points.map((p, i) => ({ time: p.time, value: Math.max(0, p.value) / stepFor(i) }))
}
...
export function buildStatusRows(latest: Record<string, number>): ServerStatusRow[] {
  const rows: ServerStatusRow[] = []
  const has = (k: string) => latest[k] != null

  if (has('node.cpu_percent')) {
    rows.push({ label: 'CPU', value: formatPercent(latest['node.cpu_percent']) })
  }
  ...
  if (has('node.network_rx_bytes_total') || has('node.network_tx_bytes_total')) {
    rows.push({
...
export function registerServerCommands(program: Command): void {
  const server = program
    .command('server')
    .description('Resource usage of the machine running the Temps control plane (what /monitoring/server shows)')

  server
    .command('status')
    .description('Latest CPU, memory, disk, block I/O and network I/O sample for the control-plane host')
  ...
  server
    .command('metrics')
    .description('Time series of one control-plane host metric, e.g. node.cpu_percent or node.network_rx_bytes_total')
    .requiredOption('--metric <name>', 'Metric name (node.cpu_percent, node.memory_used_bytes, node.disk_used_bytes, node.disk_read_bytes_total, node.network_tx_bytes_total, ...)')
  ...
  server
    ...
    .alias('df')
    .description('Docker disk usage by images, containers, volumes and build cache (docker system df) on the control-plane host')
    .option('--node <id>', 'Node ID (0 = control plane)', '0')
    .option('--json', 'Output in JSON format')
...
function parseNodeId(raw: string | undefined): number {
  ...
  if (!Number.isFinite(id) || id < 0) {
    throw new Error(`Invalid --node "${raw}": expected a non-negative integer (0 = control plane)`)
  }
  return id
}

async function serverStatusAction(options: { node?: string; json?: boolean }): Promise<void> {
  await requireAuth()
  await setupClient()
  const nodeId = parseNodeId(options.node)

  const latest = await withSpinner('Fetching latest server metrics...', async () => {
    const { data, error } = await nodeMetricsGetLatest({ client, path: { id: nodeId } })
  ...
  newline()
  header(`Server status — node ${nodeId}${nodeId === CONTROL_PLANE_NODE_ID ? ' (control plane)' : ''}`)
  if (rows.length === 0) {
    newline()
    info('No node samples yet. The first sample lands within one scrape interval (30s by default) once metric collection is enabled.')
    newline()
    return
  }
...
async function serverMetricsAction(options: {
  ...
  if (!(SERVER_RANGES as readonly string[]).includes(range)) {
    throw new Error(`Invalid --range "${range}": expected one of ${SERVER_RANGES.join(', ')}`)
  }
  ...
  if (options.json) {
    json({ node_id: nodeId, metric: options.metric, range, unit: cumulative ? 'per_second' : 'value', points: series })
    return
  }
  ...
  const fmt = (v: number) =>
    isBytes ? `${formatBytes(v)}${cumulative ? '/s' : ''}` : isPercent ? formatPercent(v) : v.toFixed(2)
...
async function serverDockerDiskUsageAction(options: { node?: string; json?: boolean }): Promise<void> {
  await requireAuth()
  await setupClient()
  const nodeId = parseNodeId(options.node)
  ...
  const usage = await withSpinner(
    ...
    async () => {
      const { data, error } = await nodeDockerDiskUsageGet({ client, path: { node_id: nodeId } })
      if (error || !data) throw new Error(getErrorMessage(error))
      return data
    }
  )
  ...
  const columns: TableColumn<Row>[] = [
    ...
    {
      header: 'Reclaimable',
      accessor: (r) => (r.cat.reclaimable_bytes == null ? '-' : formatBytes(r.cat.reclaimable_bytes)),
    },
  ]
