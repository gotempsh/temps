...
| `-c, --container <id>` | Container ID or name (partial match supported) | - | Yes |
...
| `-d, --deployment <id>` | Deployment ID, including failed retained containers | - | Yes |
| `-n, --tail <lines>` | Number of lines to tail | `1000` | Yes |
| `-t, --timestamps` | Show timestamps | - | No |
...
| Flag | Description | Default | Required |
|------|-------------|---------|----------|
| `-n, --name <name>` | Route name | - | Yes |
| `--min-severity <severity>` | Minimum severity: debug, info, warning, error, critical, emergency | - | Yes |
| `--max-severity <severity>` | Maximum severity: debug, info, warning, error, critical, emergency | - | Yes |
| `--provider-ids <ids>` | Comma-separated notification provider IDs | - | Yes |
| `--enabled <enabled>` | Enable or disable (true/false, default: true) | - | Yes |
| `--json` | Output in JSON format | - | No |
| `-y, --yes` | Skip confirmation prompts (for automation) | - | No |
...
|------|-------------|---------|----------|
| `--id <id>` | Route ID | - | Yes |
| `-n, --name <name>` | New route name | - | Yes |
| `--min-severity <severity>` | Minimum severity: debug, info, warning, error, critical, emergency | - | Yes |
| `--max-severity <severity>` | Maximum severity: debug, info, warning, error, critical, emergency | - | Yes |
| `--provider-ids <ids>` | Comma-separated notification provider IDs (replaces the current set) | - | Yes |
| `--enabled <enabled>` | Enable or disable (true/false) | - | Yes |
| `--json` | Output in JSON format | - | No |
...
| `-r, --range <window>` | Time window: 1h, 6h, 24h, 7d (default: 24h) | - | Yes |
| `-p, --percentile <n>` | Histogram percentile (0-100) instead of a plain average | - | Yes |
| `--json` | Output raw JSON instead of a formatted table | - | No |
...
| `-s, --severity <level>` | warning or critical (default: warning) | - | Yes |
| `--for-duration <secs>` | Seconds the breach must persist before firing (default: 0) | - | Yes |
...
| `-n, --name <name>` | Alert rule name | - | Yes |
| `-m, --metric <name>` | Metric name | - | Yes |
| `-c, --comparator <op>` | Comparator: >, <, >=, <= | - | Yes |
| `-t, --threshold <n>` | Threshold value | - | Yes |
| `-s, --severity <level>` | warning or critical | - | Yes |
| `--for-duration <secs>` | Seconds the breach must persist before firing | - | Yes |
...
| `--pitr <iso>` | Point-in-time recovery target, ISO 8601 timestamp (requires a PITR-capable backup). Combine with --new-service to route PITR into a new service. | - | Yes |
...
| `-p, --project-id <id>` | Project ID | - | Yes |
| `-e, --environment-id <id>` | Environment ID | - | Yes |
| `-d, --deployment-id <id>` | Only list containers belonging to this deployment | - | Yes |
| `-l, --limit <count>` | Max REPLACED container rows to return on top of the running ones, newest first (default 20, max 100) | - | Yes |
| `--json` | Output in JSON format | - | No |
...
|------|-------------|---------|----------|
| `-p, --project <project>` | Project slug or ID | - | Yes |
| `-n, --name <name>` | Operator-facing label for the key | - | Yes |
| `--environment-id <id>` | Scope the key to one environment (omit for a project-wide key) | - | Yes |
| `--allowed-origins <origins...>` | Browser origins allowed to use this key (omit to allow any origin) | - | Yes |
| `--rate-limit <n>` | Requests per minute (omit for the server default; 0 or less for unlimited) | - | Yes |
| `-y, --yes` | Skip confirmation prompts (for automation) | - | No |
| `--json` | Output in JSON format | - | No |
...
| `-n, --name <name>` | New operator-facing label | - | Yes |
| `--allowed-origins <origins...>` | Replace the origin allowlist with these origins | - | Yes |
| `--clear-origins` | Clear the origin allowlist (allow any origin) | - | No |
| `--rate-limit <n>` | New requests-per-minute limit | - | Yes |
...
| `--grant <permissions...>` | Explicitly approve declared host permissions (default: none); e.g. ai_generate projects_read | - | Yes |
| `--ai-daily-calls <count>` | Maximum AI attempts per day (0 pauses usage; default: 100) | - | Yes |
| `--ai-max-tokens <count>` | Maximum output tokens per AI call (1–4096; default: 1024) | - | Yes |
...
| Flag | Description | Default | Required |
|------|-------------|---------|----------|
| `--ref <ref>` | Use a different branch, tag, or commit | - | Yes |
...
| Flag | Description | Default | Required |
|------|-------------|---------|----------|
| `--grant <permissions...>` | Complete list of permissions to grant | - | Yes |
| `--ai-daily-calls <count>` | Daily AI call limit (0–10000) | - | Yes |
