...
- Update documentation in `docs/` when adding or changing user-facing features -- code without docs is incomplete
...
Every new feature or modification to existing behavior must follow this sequence:
1. **Implement** the code changes
2. **Test** -- write tests and verify they pass
3. **Document** -- update `docs/` if the feature is user-facing (new env vars, config options, behavior changes, API endpoints). If docs already exist for the feature, update them to match the new behavior. Code without docs is incomplete.
4. **Compile check** -- `cargo check --lib` passes with zero warnings
