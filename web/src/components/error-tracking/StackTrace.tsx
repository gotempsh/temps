...
import { codeLanguageForFile } from '@/lib/code-language'
    ...
    <div className="mt-1 rounded-md overflow-hidden border border-border/50 bg-muted text-xs font-mono">
      ...
      {pre_context?.map((line, i) => {
        ...
        return (
          <div
            key={`pre-${lineNum}`}
            className="flex hover:bg-muted-foreground/5"
          >
            <span
              className="flex-shrink-0 w-[var(--gutter)] text-right pr-3 pl-2 py-px text-muted-foreground/40 select-none border-r border-border/30 bg-background/50"
              style={
                { '--gutter': `${gutterWidth + 2}ch` } as React.CSSProperties
              }
            >
              {lineNum}
            </span>
            <pre className="flex-1 pl-3 py-px text-muted-foreground/60 overflow-x-auto">
              <HighlightedCode
                code={line || ' '}
                language={codeLanguageForFile(
                  frame.original_filename ?? frame.filename
                )}
              />
            </pre>
          </div>
      ...
      {context_line != null && (
        <div className="flex bg-red-500/10 border-l-2 border-red-500">
          <span
            className="flex-shrink-0 w-[var(--gutter)] text-right pr-3 pl-2 py-px text-red-400/70 select-none border-r border-red-500/30 bg-red-500/5"
            style={
              { '--gutter': `${gutterWidth + 2}ch` } as React.CSSProperties
            }
          >
          ...
          <pre className="flex-1 pl-3 py-px text-foreground overflow-x-auto">
            <HighlightedCode
              code={context_line || ' '}
              language={codeLanguageForFile(
                frame.original_filename ?? frame.filename
...
export function StackTrace({
  ...
  const reversedFrames =
    frames && frames.length > 0 ? [...frames].reverse() : []
              ...
              <div className="flex items-center gap-1.5 flex-shrink-0">
                {frame.symbolicated && (
                  <Code2
                    ...
                    aria-label="Symbolicated via source map"
                  />
                )}
                {isInApp && (
