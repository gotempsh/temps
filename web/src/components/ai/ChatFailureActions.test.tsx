...
test('authentication recovery offers local refresh and explicit retry', () => {
  ...
  expect(render('harness_authentication_required', 'opencode')).toContain(
...
test('rate and quota failures do not suggest credential rotation', () => {
  expect(render('provider_rate_limited')).not.toContain(
    'Refresh from local login'
  )
...
test('an active turn disables recovery mutations', () => {
  expect(render('harness_authentication_required', 'codex', true)).toMatch(
    /disabled=""/
  )
})
