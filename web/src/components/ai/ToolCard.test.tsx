...
test('completed tools stop spinning and keep output collapsed', () => {
  const html = renderToStaticMarkup(
    <ToolCard tool={{ ...tool, result: 'all tests passed' }} />
  )
...
test('Claude exit receipts show nonzero failures without expanding', () => {
  const html = renderToStaticMarkup(
    <ToolCard tool={{ ...tool, result: 'Exit code 7\ncontrolled failure' }} />
  )
  expect(html).toContain('Failed')
  expect(html).toContain('controlled failure')
  expect(html).not.toContain('animate-spin')
  const success = renderToStaticMarkup(
    <ToolCard tool={{ ...tool, result: 'Exit code 0\nok' }} />
  )
...
test('managed process failure stays visible without expanding transport details', () => {
  const html = renderToStaticMarkup(
    <ToolCard
      tool={{
        id: 'process-call',
        name: 'temps_process_stop',
        arguments: '{"process_id":"process-1"}',
        result: JSON.stringify({
          is_error: true,
          error: 'Process process-1 was not found in this workspace.',
        }),
      }}
    />
  )
...
test('managed process receipt shows lifecycle without claiming HTTP readiness', () => {
  const html = renderToStaticMarkup(
    <ToolCard
      tool={{
        id: 'process-call',
        name: 'temps_process_start',
        arguments: '{"name":"web","program":"node"}',
        result: JSON.stringify({
          type: 'process',
          process: { id: 'process-1', status: 'running', pid: 123 },
        }),
      }}
    />
  )
...
test('process receipts distinguish exited state and bounded logs and reject unknown formats', () => {
  const processTool = {
    id: 'process-call',
    name: 'temps_process_status',
    arguments: '{}',
  }
  expect(
    processToolSummary({
      ...processTool,
      result: '{"type":"process","process":{"status":"exited"}}',
    })
  ).toBe('Exited')
  expect(
    processToolSummary({
      ...processTool,
      result: '{"type":"process","process":{"status":"cancelled"}}',
    })
  ).toBe('Stopped')
  expect(
    processToolSummary({
      ...processTool,
      result: '{"type":"process","process":{"status":"succeeded"}}',
    })
  ).toBe('Exited successfully')
  expect(
    processToolSummary({
      ...processTool,
      name: 'temps_process_logs',
      result: '{"type":"logs","lines":[{}],"truncated":true}',
    })
  ).toBe('1 log line · Limited output')
  expect(
    processToolSummary({
      ...processTool,
      result:
        '{"type":"process","process":{"status":"failed","detail":"Executable not found"}}',
    })
  ).toBe('Failed · Executable not found')
  for (const result of [
    'null',
    '{',
    '{"type":"process","process":{"status":"constructor"}}',
  ]) {
  ...
  expect(
    processToolSummary({
      ...processTool,
      name: 'bash',
      result: '{"type":"process","process":{"status":"running"}}',
    })
  ).toBeUndefined()
})
