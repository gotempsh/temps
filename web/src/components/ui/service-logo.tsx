...
const RustFsIcon: SvgIcon = (props) => (
  ...
  >
    ...
    <path
      fill="currentColor"
      d="M2 4a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v4a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2V4zm2-.5a.5.5 0 0 0-.5.5v4a.5.5 0 0 0 .5.5h16a.5.5 0 0 0 .5-.5V4a.5.5 0 0 0-.5-.5H4zM2 14a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v4a2 2 0 0 1-2 2H4a2 2 0 0 1-2-2v-4zm2-.5a.5.5 0 0 0-.5.5v4a.5.5 0 0 0 .5.5h16a.5.5 0 0 0 .5-.5v-4a.5.5 0 0 0-.5-.5H4zM6 5.5a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm0 10a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm3-10a1 1 0 1 1 0 2 1 1 0 0 1 0-2zm0 10a1 1 0 1 1 0 2 1 1 0 0 1 0-2z"
    />
  </svg>
)
...
> = {
  ...
  kv: { Icon: RedisIcon, alt: 'Redis logo' },
  s3: { Icon: RustFsIcon, alt: 'S3 / RustFS logo' },
  blob: { Icon: RustFsIcon, alt: 'RustFS logo' },
  rustfs: { Icon: RustFsIcon, alt: 'RustFS logo' },
  minio: { Icon: MinioIcon, alt: 'MinIO logo' },
}

interface ServiceLogoProps extends React.HTMLAttributes<HTMLSpanElement> {
  service: ServiceTypeRoute
  size?: number
  /** No longer used — icons inherit `currentColor`. Kept for API compatibility. */
...
export function ServiceLogo({
  ...
  const entry = serviceIcons[service]
  // Only apply default size when neither a `size` prop nor an explicit
  // width/height className was provided. This lets callers use Tailwind
  // size utilities like `h-8 w-8` without being overridden by inline styles.
  const usesDefaultSize = size == null
  const resolvedStyle = usesDefaultSize
    ? style
    : { width: size, height: size, ...style }
  const defaultSizeClass = usesDefaultSize ? 'size-10' : ''
  ...
  if (!entry) {
    return (
      <span
        ...
        {...props}
      >
        <Database className="size-1/2" aria-hidden="true" />
      </span>
    )
  }

  const { Icon, alt } = entry
  ...
  return (
    <span
      ...
      {...props}
    >
      <Icon width="100%" height="100%" aria-hidden="true" focusable="false" />
    </span>
  )
}
