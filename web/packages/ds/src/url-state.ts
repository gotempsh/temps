...
/* ────────────────────────────────────────────────────────────────────────
   The URL is the state (`design-system/docs/requirements.md`).
 ...
 * owned by the page shell; `fresh` and `fail` are demo flags that outlive a
 * navigation (see `KEPT_ON_NAVIGATION`). Everything else here is view state:
 * it belongs to the screen currently on the page and is dropped when the
 * reader leaves it.
 */
...
/**
 * Keys that survive a navigation between records, by default.
 *
 * These are routing conventions, not view state: `p` names which record the
 * page is on (the sandbox routes on it, and the console's own router will name
 * it differently), and `fresh` / `fail` are the demo flags the sandbox uses to
 * force an empty or a failed rendering of a screen. An app with other
 * conventions passes its own list to `forNewView` rather than editing this one.
 */
export const KEPT_ON_NAVIGATION: readonly string[] = ['p', 'fresh', 'fail']
