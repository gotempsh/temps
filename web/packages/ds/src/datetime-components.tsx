...
export function DateTimeRangeField({
  ...
  const configuredAnchor =
    now instanceof Date ? now.getTime() : now === undefined ? undefined : now
  const items: StripItem[] = (quick ?? []).map((q) => {
    const gated = retentionDays !== undefined && q.hours / 24 > retentionDays
    const end = ms(to) ?? configuredAnchor ?? 0
    const next = {
    ...
    return {
      ...
      onClick: () => {
        ...
        const clickEnd = ms(to) ?? configuredAnchor ?? Date.now()
        onChange(
          toStamp(new Date(clickEnd - q.hours * 3_600_000), { precision }),
          toStamp(new Date(clickEnd), { precision })
        )
        setTouched(true)
      },
    }
