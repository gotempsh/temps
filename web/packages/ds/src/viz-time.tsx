  ...
  const { plot, excursions } = useMemo(() => {
    ...
    while (i < data.length) {
      ...
      for (let k = i; k <= j; k += 1) {
        const better =
          dir === 'above'
            ? Number(data[k][actual]) > Number(data[peak][actual])
            : Number(data[k][actual]) < Number(data[peak][actual])
        if (better) peak = k
      }
      ...
      const bound =
        dir === 'above'
          ? Number(data[peak][band.upper])
          : Number(data[peak][band.lower])
      const crossed =
        errorAt !== undefined &&
        (worse === 'down' ? value <= errorAt : value >= errorAt)
      const before = [...markers]
        .filter(
          (m) =>
            data.findIndex((p) => p.t === m.x) <= peak &&
            data.findIndex((p) => p.t === m.x) >= 0
        )
        .pop()
      out.push({
        x: String(data[peak].t),
        value,
        bound,
        pct: bound ? ((value - bound) / Math.abs(bound)) * 100 : 0,
        side: dir,
        state: crossed ? 'error' : 'warn',
        points: j - i + 1,
        deploy: before?.id,
      })
      i = j + 1
    ...
    const plot: TimePoint[] = data.map((p, n) =>
      inRun[n] ? { ...p, [OUT]: Number(p[actual]) } : p
    )
    return { plot, excursions: out }
  }, [data, actual, band.lower, band.upper, worse, errorAt, markers])

  const worst = excursions.reduce<Excursion | null>(
    (b, e) => (b === null || Math.abs(e.pct) > Math.abs(b.pct) ? e : b),
    null
  )
  ...
  return (
    <div className={cn('min-w-0 space-y-2', className)}>
      <TimeChart
        ...
        readoutFormat={(p) => {
          const lo = p[band.lower],
            hi = p[band.upper]
          const v = fmtNum(Number(p[actual]))
          const rangeWord =
            lo === undefined
              ? ''
              : ` · expected ${fmtNum(Number(lo))}–${fmtNum(Number(hi))}${u}`
      ...
      {excursions.length > 0 && (
        <ol className="op-rows border bg-background font-mono text-[11px]">
          <li className="op-label px-2 py-1 text-[9px]">
            {excursions.length} outside the expected range
          </li>
          {excursions.map((e) => {
            const p = at.get(e.x)
            return (
              <li key={e.x}>
                <button
                  ...
                  onClick={() => onOpen?.(e)}
                  className={cn(
                    'flex w-full flex-wrap items-baseline gap-x-3 px-2 py-1 text-left',
                    onOpen && 'hover:bg-muted'
                  )}
                >
                  <span aria-hidden className={GLYPH_CLASS[e.state]}>
                    ×
                  </span>
                  ...
                  <span
                    className={
                      e.state === 'error' ? 'text-destructive' : 'text-warning'
                    }
                  >
                    {e.pct >= 0 ? '+' : ''}
                    {fmtNum(e.pct, {
                      digits: Math.abs(e.pct) < 10 ? 1 : 0,
                    })}
                    % {e.side}
                  </span>
                  <span className="text-muted-foreground">
                    expected{' '}
                    {p
                      ? `${fmtNum(Number(p[band.lower]))}–${fmtNum(Number(p[band.upper]))}${u}`
                      : '—'}
                    {' · '}
...
export function StackedInk({
  ...
  if (import.meta.env.DEV && layers.length > 4)
    console.warn(
      `[chart] StackedInk has ${layers.length} layers; four patterns is the limit of what the eye separates. Use a table (data-viz.md §StackedInk).`
    )
  const totals = data.map((p) =>
    layers.reduce((a, l) => a + (Number(p[l.key]) || 0), 0)
  )
  ...
  const readText = read
    ? `${read.t} · ${layers.map((l) => `${l.name} ${fmtNum(Number(read[l.key]) || 0)}`).join(' · ')}`
    : ''
  const fillOf = (l: InkLayer, n: number) =>
    l.state ? INK_TONE[l.state] : INK_FILL[l.fill ?? INK_LAYER_ORDER[n % 4]]
  const opacityOf = (l: InkLayer, n: number) =>
    l.state ? 0.9 : INK_FILL_OPACITY[l.fill ?? INK_LAYER_ORDER[n % 4]]
  ...
  return (
    <Figure
      ...
      footer={
        <>
          {footer}
          {partial && <span>· ▨ the current bucket is still filling</span>}
        </>
      }
      legend={
            ...
            <li key={l.key} className="flex items-center gap-1.5">
              ...
              >
                <rect
                  ...
                  height={10}
                  fill={fillOf(l, n)}
                  fillOpacity={opacityOf(l, n)}
                  stroke="var(--op-rule-soft)"
                />
              </svg>
              {l.state && (
                <span aria-hidden className={GLYPH_CLASS[l.state]}>
                  {GLYPH[l.state]}
                </span>
              )}
              <span>{l.name}</span>
              <span className="tabular-nums text-foreground">
                {read ? fmtNum(Number(read[l.key]) || 0) : fmtNum(sum(l.key))}
              </span>
              <span>
                {read
                  ? ''
                  : `· ${fmtPct(grand ? sum(l.key) / grand : 0, { basis: 'ratio', digits: 1 })}`}
              </span>
              {/* The pattern is named, so the legend works read aloud too. */}
              <span className="sr-only">
                {l.fill
                  ? INK_FILL_WORD[l.fill]
                  : INK_FILL_WORD[INK_LAYER_ORDER[n % 4]]}
              </span>
            </li>
          ))}
        </ul>
      ...
      table={
        <DataTable
          ...
          head={[
            'bucket',
            ...layers.map((l) => `${l.name}${unit ? ` (${unit})` : ''}`),
            'total',
          ]}
          numeric={layers.map((_, n) => n + 1).concat([layers.length + 1])}
          rows={data.map((p, n) => [
            p.t,
            ...layers.map((l) => fmtNum(Number(p[l.key]) || 0)),
            fmtNum(totals[n]),
          ])}
        />
      }
    >
      <div className="min-w-0">
        ...
        <div
          {...regionProps}
          aria-label={`${data.length} buckets · use arrow keys to read each`}
          className={cn(regionProps.className, 'mt-1')}
        >
          <svg
                  ...
                  {layers.map((l, li) => {
                    ...
                    return h > 0 ? (
                      <rect
                        ...
                        height={h}
                        fill={fillOf(l, li)}
                        fillOpacity={opacityOf(l, li)}
                      />
                    ) : null
                  })}
                  {partial && n === data.length - 1 && (
                    <rect
                      x={n * 10 + 0.6}
                      y={100 - acc}
                      width={8.8}
...
export function LatencyHeatmap({
  ...
  const rowLabel = (r: LatencyRow, n: number) =>
    r.label ?? `${n === 0 ? '0' : fmtNum(rows[n - 1].le)}–${fmtNum(r.le)}`
  ...
  const yOf = (v: number) => {
    ...
    const frac = Math.max(
      0,
      Math.min(1, (v - lower) / Math.max(1e-9, rows[r].le - lower))
    )
    return ((r + frac) / rows.length) * 100
  }
  const read = at
    ? `${columns[at.c]} · ${rowLabel(rows[at.r], at.r)}${unit} · ${fmtNum(counts[at.r][at.c])} requests`
    : ''
  ...
  const move = (dr: number, dc: number) =>
    setAt((p) => {
      ...
      return {
        r: Math.max(0, Math.min(rows.length - 1, n.r + dr)),
        c: Math.max(0, Math.min(columns.length - 1, n.c + dc)),
      }
    })
  ...
  return (
    <Figure
            ...
            {[0, 1, 2, 3, 4].map((s) => (
              <span
                ...
                style={{
                  ...
                  opacity: [0.06, 0.22, 0.42, 0.68, 1][s],
                }}
              />
            ))}{' '}
            {fmtNum(max)}
            ...
            <span key={o.name} className="flex items-center gap-1.5">
              ...
              >
                <line
                  ...
                  stroke="var(--foreground)"
                  strokeWidth={1}
                  strokeDasharray={DASH[o.stroke ?? 'dashed']}
                />
              </svg>
              {o.name}
            </span>
      ...
      table={
        <DataTable
          caption={label}
          head={[`latency (${unit})`, ...columns]}
          numeric={columns.map((_, n) => n + 1)}
          rows={rows.map((r, n) => [
            rowLabel(r, n),
            ...counts[n].map((c) => fmtNum(c)),
          ])}
        />
      }
    >
      <div className="min-w-0">
        <div className="flex items-baseline justify-between font-mono text-[11px]">
          <span className="tabular-nums">
            {read || `${fmtNum(total)} requests · ${range}`}
          </span>
        ...
        <div className="mt-1 flex min-w-0 gap-2">
          ...
          <div
            role="group"
            tabIndex={0}
            aria-label={`${rows.length} by ${columns.length} grid · use arrow keys to read a cell`}
            onFocus={() => setAt((p) => p ?? { r: rows.length - 1, c: 0 })}
            onBlur={() => setAt(null)}
            onKeyDown={(e) => {
              const k = {
                ArrowUp: [-1, 0],
          ...
          >
                  ...
                  {overlays.map((o) => (
                    <polyline
                      ...
                      points={o.values
                        .map(
                          (v, n) =>
                            `${((n + 0.5) / columns.length) * 100},${yOf(v)}`
                        )
                        .join(' ')}
                    />
                  ))}
            ...
            <div className="mt-1 flex justify-between font-mono text-[10px] text-muted-foreground">
              {columns
                .filter(
                  (_, n) =>
                    n % Math.max(1, Math.floor(columns.length / 4)) === 0
                )
                .map((c) => (
                  <span key={c}>{c}</span>
                ))}
  ...
  return (
    <Figure
        ...
        <ul className="flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[10px] text-muted-foreground">
          {words.map((w) => (
            <li key={w.word} className="flex items-center gap-1.5">
              <span aria-hidden className={GLYPH_CLASS[w.state]}>
                {GLYPH[w.state]}
              </span>
              {w.word}
              <span className="tabular-nums text-foreground">
                {fmtPct(w.seconds / total, { basis: 'ratio', digits: 2 })}
              </span>
      ...
      table={
        <DataTable
          caption={label}
          head={['from', 'state', 'for', 'share', 'why']}
          numeric={[2, 3]}
          rows={segments.map((s) => [
            s.from,
            <span key="s" className={GLYPH_CLASS[s.state]}>
              {GLYPH[s.state]} {s.word}
            </span>,
            fmtDuration(s.seconds * 1000),
            fmtPct(s.seconds / total, { basis: 'ratio', digits: 2 }),
            s.note ?? '—',
          ])}
        />
      }
    >
      <div className="min-w-0">
        <div className="flex items-baseline justify-between font-mono text-[11px]">
          <span className="tabular-nums">
            {readText || `${range} · ${segments.length} state changes`}
          </span>
          <span className="text-muted-foreground">
            {seg ? 'segment' : 'window'}
          </span>
        ...
        <div
          {...regionProps}
          aria-label={`${segments.length} segments · use arrow keys to read each`}
          className={cn(regionProps.className, 'mt-1 flex gap-px')}
          style={{ height }}
        >
          {segments.map((s, n) => (
            <span
...
export function WindowTimeline({
  ...
  const pos = (iso: string) =>
    Math.max(0, Math.min(100, ((Date.parse(iso) - t0) / span) * 100))
  const { i, setI, regionProps } = useReadout(marks.length)
  const mark = i === null ? null : marks[i]
  const readText = mark
    ? `${mark.label} · ${fmtAbsolute(mark.at, { seconds: true })}${zone ? ` ${zone}` : ''}${mark.note ? ` · ${mark.note}` : ''}`
    : ''
  const outside =
    cursor && (Date.parse(cursor.at) < t0 || Date.parse(cursor.at) > t1)
  ...
  return (
    <Figure
        ...
        <ul className="flex flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[10px] text-muted-foreground">
          <li className="flex items-center gap-1.5">
            <svg aria-hidden width={14} height={8} viewBox="0 0 14 8">
              <rect
                width={14}
                height={8}
                fill="url(#op-hatch-soft)"
                stroke="var(--op-rule-soft)"
              />
            </svg>
            {covered[0]?.label ?? 'covered'}
          </li>
          <li className="flex items-center gap-1.5">
          ...
          {cursor && (
            <li className="flex items-center gap-1.5">
              <span
                aria-hidden
                className={outside ? 'text-destructive' : 'text-foreground'}
              >
                ▼
              </span>
              {cursor.label}{' '}
              <span className="tabular-nums text-foreground">
                {fmtAbsolute(cursor.at, { seconds: true })}
                {zone ? ` ${zone}` : ''}
              </span>
            </li>
          )}
      ...
      table={
        <DataTable
          caption={label}
          head={['backup', 'taken', 'state', 'note']}
          rows={marks.map((m) => [
            m.label,
            `${fmtAbsolute(m.at, { seconds: true })}${zone ? ` ${zone}` : ''}`,
            m.state ? (
              <span key="s" className={GLYPH_CLASS[m.state]}>
                {GLYPH[m.state]} {m.state}
              </span>
            ) : (
              'ok'
            ),
      ...
      <div className="min-w-0">
        <div className="flex items-baseline justify-between font-mono text-[11px]">
          <span className="tabular-nums">
            {readText ||
              (cursor
                ? `${cursor.label} ${fmtAbsolute(cursor.at, { seconds: true })}${zone ? ` ${zone}` : ''}`
                : title)}
          </span>
          <span className="text-muted-foreground">
            {mark ? 'backup' : cursor ? 'target' : ''}
          </span>
        </div>
        <div
          {...regionProps}
          aria-label={`${marks.length} backups · use arrow keys to read each`}
          className={cn(regionProps.className, 'relative mt-1 border')}
          style={{ height }}
        >
          {covered.map((c) => (
            <span
              key={c.from}
              aria-hidden
              className="op-ink-hatch absolute inset-y-0"
              style={{
                left: `${pos(c.from)}%`,
                width: `${pos(c.to) - pos(c.from)}%`,
              }}
            />
          ))}
          {marks.map((m, n) => (
            <span
              ...
              className={cn(
                ...
                m.state === 'error' ? 'bg-destructive' : 'bg-foreground',
                i === n && 'w-0.5'
              )}
              style={{ left: `${pos(m.at)}%` }}
            />
          ...
          {cursor && !outside && (
            <span
              ...
              style={{ left: `${pos(cursor.at)}%` }}
            >
              <span className="absolute -top-px left-1/2 -translate-x-1/2 font-mono text-[9px] leading-none">
                ▼
              </span>
            </span>
          )}
        ...
        <div className="mt-1 flex justify-between font-mono text-[10px] text-muted-foreground">
          <span>{fmtAbsolute(from)}</span>
          {zone && <span>{zone}</span>}
          <span>{fmtAbsolute(to)}</span>
        </div>
        <ReadoutLive text={readText} />
      </div>
    </Figure>
...
const KIND_GLYPH: Record<SessionEvent['kind'], string> = {
  pageview: '▤',
  click: '◆',
  input: '▮',
  network: '↔',
...
export function SessionTimeline({
  ...
  const pos = (ms: number) =>
    Math.max(0, Math.min(100, (ms / Math.max(1, duration_ms)) * 100))
  const current = events.reduce(
    (best, e, n) => (e.at_ms <= position_ms ? n : best),
    -1
  )
  const sel = i === null ? null : events[i]
  const readText = sel
    ? `${offset(sel.at_ms)} · ${sel.kind} · ${sel.label}${sel.note ? ` · ${sel.note}` : ''}`
    : ''
  ...
  return (
    <div className={cn('min-w-0 space-y-2', className)}>
      <div className="flex items-baseline justify-between font-mono text-[11px]">
        <span className="tabular-nums">
          {readText || `${offset(position_ms)} of ${fmtDuration(duration_ms)}`}
        </span>
        <span className="text-muted-foreground">
          {sel ? 'event' : 'position'}
        </span>
      ...
      <div
        role="img"
        aria-label={label}
        className="relative h-8 border bg-background"
      >
          ...
          {events.map((e, n) => {
            ...
            return (
              <span
                ...
                style={{ left: `${pos(e.at_ms)}%` }}
              >
                <span
                  className={cn(
                    'w-px',
                    boundary || e.state ? 'h-full' : 'h-2/5',
                    tone,
                    (e.state || i === n) && 'w-0.5',
                    i === n && !e.state && 'bg-foreground'
                  )}
                />
              </span>
            )
      ...
      <div className="px-2">
        <input
          ...
          value={position_ms}
          disabled={!onSeek}
          aria-label={`position in the session, ${offset(position_ms)} of ${fmtDuration(duration_ms)}`}
          onChange={(e) => onSeek?.(Number(e.target.value))}
          className="block h-1 w-full accent-[var(--foreground)]"
        />
      </div>
      ...
      <div
        {...regionProps}
        aria-label={`${events.length} events · arrow keys step, enter seeks`}
        className={cn(
          regionProps.className,
          'max-h-56 overflow-auto border bg-background'
        )}
        onKeyDown={(e) => {
          if (e.key === 'Enter' && i !== null) {
            e.preventDefault()
            onSeek?.(events[i].at_ms)
          } else regionProps.onKeyDown(e)
        }}
      >
          ...
          {events.map((e, n) => (
            <li key={n}>
              <button
                ...
                className={cn(
                  'flex w-full items-baseline gap-3 px-2 py-1 text-left',
                  onSeek && 'hover:bg-muted',
                  (i === n || (i === null && current === n)) && 'bg-muted'
                )}
              >
                ...
                <span
                  ...
                  className={cn(
                    ...
                    e.state ? GLYPH_CLASS[e.state] : 'text-muted-foreground'
                  )}
                >
                  {KIND_GLYPH[e.kind]}
                </span>
