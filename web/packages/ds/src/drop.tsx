...
export function Drop({
  ...
  const [fixed, setFixed] = useState<{ top?: number; bottom?: number } | null>(
    null
  )
  useLayoutEffect(() => {
    if (!open) return
    const place = () => {
      ...
      setFixed(
        side === 'above'
          ? { bottom: Math.max(12, window.innerHeight - r.top + 6) }
          : { top: r.bottom + 6 }
      )
    }
    place()
  ...
  return (
    <div
      ...
      className={cn(
        'z-30 border bg-background',
        !fixed && 'absolute right-0',
        !fixed && (side === 'above' ? 'bottom-9' : 'top-9'),
        className
      )}
    >
      {children}
