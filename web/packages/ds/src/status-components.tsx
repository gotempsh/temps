...
import { type State, GLYPH, glyphClass, type StatusItem } from './status-shared'
...
export function Status({
  ...
  return (
    ...
    >
      <span aria-hidden className={cn('w-3 text-center', glyphClass(state))}>
        {GLYPH[state]}
      </span>
      {label}
    </span>
...
export function StatusLine(props: {
  state: State
  children: ReactNode
  more?: { label: string; onClick?: () => void; items?: StatusItem[] }
  sticky?: boolean
  className?: string
}) {
  const slots = useShellSlots()
  if (slots?.attention)
    return createPortal(<AttentionEntry {...props} />, slots.attention)
    ...
    <>
      {all.map((it, i) => (
        ...
        >
          <span
            aria-hidden
            className={cn('w-3 shrink-0 text-center', glyphClass(it.state))}
          >
            {GLYPH[it.state]}
          </span>
      ...
      {more?.onClick && !more.items?.length && (
        <li className="py-0.5 pl-6 text-xs">
          <a
            href="#"
            onClick={(e) => {
              e.preventDefault()
...
export function AttentionHost({
  ...
  const setList = (el: HTMLUListElement | null) => {
    list.current = el
    onSlot(el)
  }
  useEffect(() => {
    ...
    const count = () => {
      const states = [
        ...el.querySelectorAll<HTMLElement>('li[data-state]'),
      ].map((li) => li.dataset.state as State)
      setCounts({
        errors: states.filter((s) => s === 'error').length,
        warns: states.filter((s) => s === 'warn' || s === 'sampled').length,
        total: states.length,
      })
    }
    count()
  ...
  const label = quiet
    ? counts.total
      ...
      : 'no verdict on this page'
    : `${counts.errors ? `${counts.errors} error${counts.errors > 1 ? 's' : ''}` : ''}${counts.errors && counts.warns ? ', ' : ''}${counts.warns ? `${counts.warns} warning${counts.warns > 1 ? 's' : ''}` : ''}`
  return (
    <div ref={ref} className="relative">
      <button
        ...
        title={label}
        className={cn(
          'flex h-7 items-center gap-2 border px-2 font-mono text-[11px] tabular-nums hover:bg-muted',
      ...
      >
        {quiet ? (
          <span
            aria-hidden
            className={counts.total ? glyphClass('ok') : glyphClass('idle')}
          >
            {counts.total ? GLYPH.ok : GLYPH.idle}
          </span>
        ) : (
          <>
            {counts.errors > 0 && (
              <span className="flex items-center gap-1">
                <span aria-hidden className={glyphClass('error')}>
                  {GLYPH.error}
                </span>
                {counts.errors}
              </span>
            ...
            {counts.warns > 0 && (
              <span className="flex items-center gap-1">
                <span aria-hidden className={glyphClass('warn')}>
                  {GLYPH.warn}
                </span>
                {counts.warns}
              </span>
      ...
      <Drop
        ...
        role="dialog"
        label="attention"
        className={cn('op-status text-sm leading-6', !open && 'hidden')}
      >
        <div className="flex items-center justify-between border-b px-3 py-1.5">
          <span className="op-label">attention</span>
          <span className="text-[11px] text-muted-foreground">{label}</span>
        </div>
        <ul ref={setList} tabIndex={-1} className="px-3 py-2 outline-none">
          {counts.total === 0 && (
            <li className="py-0.5 text-xs text-muted-foreground">
              this page has no verdict
            </li>
          )}
        </ul>
      </Drop>
...
export function StatusLineInline({
  state,
  children,
  more,
  sticky = true,
  ...
  return (
    <div
      className={cn(
        'op-status -mx-4 overflow-hidden border-b px-4 py-2 text-sm leading-6 sm:-mx-6 sm:px-6',
        sticky && 'op-sticky',
        className
      )}
    >
      <p className="flex items-baseline gap-3">
        <span
          aria-hidden
          className={cn('w-3 shrink-0 text-center', glyphClass(state))}
        >
          {GLYPH[state]}
        </span>
