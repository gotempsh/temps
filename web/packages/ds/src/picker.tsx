    ...
    >
      <PopoverTrigger asChild>
        <button
          ...
          className={cn(
            'flex h-8 w-full items-center gap-2 border bg-background px-2 text-left text-xs hover:bg-muted focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring',
            mono && 'font-mono',
          ...
          {selected?.state && (
            <span
              ...
              className={cn(
                ...
                GLYPH_CLASS[selected.state]
              )}
            >
              {GLYPH[selected.state]}
            </span>
          )}
          {selected?.icon && (
            <span
              aria-hidden
              className="flex w-3.5 shrink-0 items-center justify-center text-muted-foreground [&_svg]:h-3.5 [&_svg]:w-3.5"
            >
              {selected.icon}
            </span>
          )}
      ...
      <PopoverContent
        align="start"
        className={cn(skin, 'border p-0 shadow-none sm:rounded')}
        style={{
          width:
            width ??
            'min(calc(100vw - 2rem), max(380px, var(--radix-popover-trigger-width)))',
        }}
      >
        <Command className={cn('rounded-none bg-popover', mono && 'font-mono')}>
          ...
          <CommandList className="max-h-72 text-xs">
            {loading && (
              <p className="flex items-center gap-2 px-3 py-3 text-muted-foreground">
                <span aria-hidden>◌</span> loading {loading}
              </p>
            )}
            {error && (
              <div className="space-y-2 px-3 py-3">
                ...
                {onRetry && (
                  <button
                    type="button"
                    onClick={onRetry}
                    className="inline-flex h-7 items-center gap-1 border px-2 hover:bg-muted"
                  >
                ...
                {allowCustom && (
                  <p className="text-[11px] text-muted-foreground">
                    or type a value above and choose &quot;{allowCustom}&quot;.
                  </p>
                )}
              </div>
            )}
            {!loading && !error && (
              <CommandEmpty className="px-3 py-3 text-left text-muted-foreground">
                {custom ? null : `nothing matches "${typed}"`}
              </CommandEmpty>
            )}
            {!loading &&
              !error &&
              groups.map(([g, items]) => (
                <CommandGroup
                  key={g || '__'}
                  heading={g || undefined}
                  className="[&_[cmdk-group-heading]]:py-1 [&_[cmdk-group-heading]]:text-[10px] [&_[cmdk-group-heading]]:font-medium [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-[0.1em]"
                >
                  {items.map((o) => (
                    <CommandItem
                      key={o.value}
                      value={`${o.label ?? o.value} ${o.value} ${o.keywords ?? ''}`}
                      disabled={o.disabled}
                      onSelect={() => choose(o.value)}
                      className="gap-2 rounded-none data-[selected=true]:bg-foreground data-[selected=true]:text-background"
                    >
                      ...
                      {o.icon && (
                        <span
                          aria-hidden
                          className={cn(
                            'flex w-3.5 shrink-0 items-center justify-center [&_svg]:h-3.5 [&_svg]:w-3.5',
                            o.value === value ? '' : 'text-muted-foreground'
                          )}
                        >
                          {o.icon}
                        </span>
                      ...
                      {o.meta && (
                        <span
                          className="min-w-0 flex-1 truncate text-right text-[11px] opacity-60"
                          title={
                            typeof o.meta === 'string' ? o.meta : undefined
                          }
                        >
                          {o.meta}
                        </span>
            ...
            {custom && (
              ...
              >
                ...
                >
                  <span className="text-muted-foreground group-data-[selected=true]:text-inherit">
                    {allowCustom}
                  </span>{' '}
          ...
          <div className="flex items-center gap-2 whitespace-nowrap border-t px-3 py-1.5 text-[10px] text-muted-foreground">
            <Kbd keys="↑↓" /> move <Kbd keys="⏎" className="ml-1" /> choose{' '}
            <Kbd keys="esc" className="ml-1" /> close
          </div>
        </Command>
      </PopoverContent>
    </Popover>
