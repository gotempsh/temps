...
export function SecretValue({
  ...
  return (
    <span
      className={cn('flex min-w-0 items-center gap-1 font-mono', className)}
    >
      <span
        className={cn(
          'min-w-0 truncate',
          !shown && 'tracking-wider text-muted-foreground'
        )}
        aria-label={shown ? undefined : 'hidden secret value'}
      >
        {shown ? value : '••••••••••••'}
      </span>
      {secret && (
