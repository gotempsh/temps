...
export function AgentGlyph({
  ...
  return (
    <span
      aria-hidden
      className={cn('w-3 shrink-0 text-center', GLYPH_CLASS[state], className)}
    >
      {GLYPH[state]}
    </span>
  )
...
export function AgentRow({
  ...
  return (
    <div
      className={cn(
        accent === 'destructive' && 'border-s-2 border-destructive ps-2',
        className
      )}
    >
      <button
        ...
        onClick={onToggle}
        aria-expanded={open}
        className="group -mx-1 flex min-h-7 w-[calc(100%+0.5rem)] items-center gap-2 px-1 py-1 text-start text-xs hover:bg-muted"
      >
        <AgentKindIcon icon={icon} state={state} />
        <span className="min-w-0 flex-1 truncate [&>.break-all]:whitespace-normal">
          {title}
        </span>
        {meta && (
          <span
        ...
        {children !== undefined &&
          (open ? (
            <ChevronDown className="h-3 w-3 shrink-0 opacity-40 group-hover:opacity-70" />
          ) : (
            <ChevronRight className="h-3 w-3 shrink-0 opacity-40 group-hover:opacity-70" />
          ))}
      </button>
...
export function AgentInset({
  ...
  return (
    <div
      className={cn(
        'op-inset my-1 px-2 py-1.5 font-mono text-[11px] leading-5',
        className
      )}
    >
        ...
        return (
          <div
            ...
            className={cn(
              'flex gap-2',
              k === 'del' &&
                'text-muted-foreground line-through decoration-[var(--op-rule-soft)]',
              k === 'hunk' && 'text-muted-foreground',
              k === 'ctx' && 'text-muted-foreground'
            )}
          >
            <span
              aria-hidden
              className={cn(
                'w-3 shrink-0 select-none text-center no-underline',
                k === 'add' && 'text-success',
...
export function ToolRow({
  ...
  useEffect(() => {
    ...
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === 'y' || e.key === 'Y') {
        e.preventDefault()
        approval.onRespond('once')
      } else if (e.key === 'n' || e.key === 'N') {
        e.preventDefault()
        approval.onRespond('deny')
      }
    }
  ...
  return (
    <AgentRow
      ...
      title={
        <span
          className={cn('font-mono', isCmd && 'whitespace-normal break-all')}
        >
          <span className="font-medium">{isCmd ? '$' : name}</span>
          {arg && (
            <span
              className={cn(
                isCmd ? 'text-foreground' : 'text-muted-foreground'
              )}
            >
              {' '}
              {arg}
      ...
      meta={
        meta ?? (
          <>
            {approved && state === 'output-available' ? 'approved · ' : ''}
            {s.word}
            {ms !== undefined &&
              (state === 'output-available' || state === 'input-available') &&
              ` · ${dur(ms)}`}
            {state === 'input-streaming' && <span className="op-caret" />}
          </>
        )
      }
      accent={
        approval?.destructive && state === 'approval-requested'
          ? 'destructive'
          : undefined
      }
    >
        ...
        <div className="space-y-2 py-1 text-xs">
          ...
          <div className="flex flex-wrap gap-2">
            <Button
              ...
              className={cn(
                'h-7 text-xs',
                approval.destructive ? 'op-fill-destructive' : 'op-primary'
              )}
              onClick={() => approval.onRespond('once')}
            >
              {approval.destructive ? 'run it' : 'approve'}{' '}
              <Kbd keys="Y" className="ms-1 opacity-70" />
            </Button>
            {!approval.destructive && (
              <Button
                ...
                onClick={() => approval.onRespond('session')}
              >
            ...
            <Button
              ...
              variant="outline"
              className="h-7 text-xs"
              onClick={() => approval.onRespond('deny')}
            >
              deny <Kbd keys="N" className="ms-1 opacity-70" />
            </Button>
            <span className="ms-auto self-center text-[11px] text-muted-foreground">
              the agent waits; nothing runs until you answer
            </span>
          </div>
        </div>
      )}
      {state === 'output-denied' && (
        <div className="py-1 text-xs text-muted-foreground">
          denied · the agent was told why and will try another way
        </div>
      )}
      {output !== undefined && state === 'output-available' && !diff && (
        <AgentInset label={isCmd ? undefined : 'output'}>{output}</AgentInset>
      )}
      {error && state === 'output-error' && (
        <AgentInset label="error" className="text-destructive">
          {error}
        </AgentInset>
      )}
      {children}
      ...
      <p className="flex flex-wrap items-baseline gap-x-2 gap-y-1 font-mono text-[11px] text-muted-foreground">
        ...
        {query && (
          <button
            ...
            aria-expanded={open}
            onClick={() => setOpen((o) => !o)}
            className="ms-auto underline decoration-[var(--op-rule-soft)] underline-offset-4 hover:text-foreground"
          >
  ...
  return (
    <div className={cn('border', pending && 'op-raise', className)}>
      <p className="flex items-center gap-2 border-b px-2 py-1.5 text-xs">
        <AgentKindIcon
          icon={toolIcon(kind)}
          state={pending ? 'warn' : decided === 'confirmed' ? 'ok' : 'idle'}
        />
        ...
        <span className="ms-auto shrink-0 font-mono text-[11px] text-muted-foreground">
          {pending ? 'waiting for you' : decided}
        </span>
      </p>
          ...
          <>
            {irreversible ? (
              <EchoDialog
                destructive
                trigger={
                  <Button size="sm" className="op-fill-destructive h-7 text-xs">
                    {confirmLabel ?? action}
                  </Button>
                }
                title={`${action} ${target}`}
              ...
              <Button
                size="sm"
                className="op-primary h-7 text-xs"
                onClick={onConfirm}
              >
                {confirmLabel ?? action}
              </Button>
            ...
            <span className="ms-auto">
              nothing has run · the agent waits for you
            </span>
          </>
        ) : decided === 'confirmed' ? (
          <Status
            state="ok"
            label={`confirmed · you approved this, the agent ran it`}
            className="font-mono"
          />
        ) : (
          <Status
    ...
    >
      ...
      {kind === 'chart' && (
        <div className="border p-2">
          <div className="flex h-[140px] items-end gap-1" aria-hidden>
            {[38, 52, 44, 61, 57, 70, 48, 66, 59, 74, 63, 51].map((h, i) => (
              <div
                key={i}
      ...
      {kind === 'ledger' && (
        <div className="border">
          {[0, 1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="flex items-center gap-3 border-b border-[var(--op-rule-soft)] px-3 py-2 last:border-b-0"
            >
              <div className={cn(BAR, 'w-3')} />
              <div className={cn(BAR, 'w-2/5')} />
              <div className={cn(BAR, 'ms-auto w-16')} />
            </div>
          ))}
        </div>
      )}
      ...
      {kind === 'tool' && (
        <div className="flex min-h-7 items-center gap-2">
          ...
          <div className={cn(BAR, 'w-1/3')} />
          <div className={cn(BAR, 'ms-auto w-14')} />
        </div>
      )}
      <p className="font-mono text-[11px] text-muted-foreground">
        {label ?? `drawing the ${kind}`}
        <span className="op-caret" aria-hidden />
      </p>
    </div>
  )
...
export function AgentQuestion({
  ...
  answer,
  onAnswer,
  hint = 'or type an answer below · the agent waits',
}: {
  q: string
  ...
  return (
    <div
      className={cn('border', !answer && 'op-raise')}
      role="radiogroup"
      aria-label={q}
    >
      <p className="flex items-center gap-2 border-b px-2 py-1.5 text-xs">
      ...
      <div className="grid gap-px sm:grid-cols-2">
        {options.map((o, i) => (
          <button
            ...
            type="button"
            role="radio"
            aria-checked={chosen === o.label}
            disabled={!!answer}
            onClick={() => setPicked(o.label)}
            className={cn(
              'flex items-start gap-2 px-2 py-2 text-start text-xs',
              !answer && 'hover:bg-muted',
              answer === o.label && 'op-fill-ink border-0',
              !answer && picked === o.label && 'bg-muted',
              answer && answer !== o.label && 'text-muted-foreground'
            )}
          >
            <span aria-hidden className="mt-px w-3 text-center">
              {chosen === o.label ? '●' : '○'}
            </span>
      ...
      {!answer && (
        <div className="flex flex-wrap items-center gap-2 border-t px-2 py-1.5 text-[11px] text-muted-foreground">
          ...
          >
            {picked ? <>confirm “{picked}”</> : 'pick an answer'}{' '}
            <Kbd keys="⏎" className="ms-1 opacity-70" />
          </Button>
          {picked && (
            <button
              type="button"
  ...
  const all: KV[] = [
    ...
    ...(context
      ? [
          {
            k: 'context',
            v: `${fmtNum(context.used)} / ${fmtNum(context.max / 1000, { digits: 0 })}k · ${pct}%`,
            state: (pct >= 90 ? 'error' : pct >= 75 ? 'warn' : 'ok') as State,
          },
        ]
      : []),
    ...(checkpoints
      ? [
          {
            ...
            v:
              checkpoints.length === 0
                ? 'none yet'
                : `${checkpoints.length} · last ${last?.at}`,
          },
        ]
      : []),
    ...(rows ?? []),
