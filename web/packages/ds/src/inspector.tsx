...
export function Inspector({
  ...
  useEffect(() => {
    ...
    const onKey = (e: globalThis.KeyboardEvent) => {
      ...
      if (Number.isInteger(n) && n >= 1 && n <= anchors.length) {
        e.preventDefault()
        jump(anchors[n - 1].id)
      }
    }
    ...
    <>
      ...
      >
        <div className="flex min-w-0 shrink-0 items-center gap-2 border-b px-3 py-2 text-xs">
          ...
          <span aria-live="polite" className="min-w-0 truncate font-mono">
            {title}
          </span>
          {meta && (
            <span className="hidden min-w-0 shrink-[2] truncate font-mono text-[11px] text-muted-foreground sm:block">
              {meta}
            </span>
          )}
          <span className="ms-auto flex shrink-0 items-center gap-1">
            {onOpen && (
              <button
                type="button"
                onClick={onOpen}
                className="inline-flex h-6 items-center border px-2 text-[11px] hover:bg-muted"
              >
                {openLabel}
              </button>
            )}
            {copyLink && (
              <CopyAction value={copyLink} onCopied={onCopied}>
                copy link
              </CopyAction>
            )}
            <button
        ...
        {anchors.length > 0 && (
          <nav
            ...
            className="flex shrink-0 flex-wrap items-center gap-x-2 gap-y-1 border-b px-3 py-1.5 text-[11px] text-muted-foreground"
          >
            {anchors.map((a, i) => (
              <button
                key={a.id}
