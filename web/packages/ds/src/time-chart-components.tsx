...
const DASH: Record<SeriesStroke, string | undefined> = {
  solid: undefined,
  dashed: '4 3',
  dotted: '1 3',
}
...
const strokeOf = (s: Series, i: number): SeriesStroke =>
  s.stroke ?? AUTO[i % AUTO.length]

const widthOf = (s: Series, i: number) =>
  s.width ?? (s.weight ? (s.weight === 'thin' ? 1 : 1.5) : i === 0 ? 1.5 : 1)
    ...
    <ul className="flex min-w-0 flex-wrap items-center gap-x-3 gap-y-1 font-mono text-[10px] text-muted-foreground">
        ...
        <li key={s.key} className="flex items-center gap-1.5">
          ...
          >
            <line
              ...
              y2={3}
              stroke={colorOf(s)}
              strokeWidth={widthOf(s, i)}
              strokeDasharray={DASH[strokeOf(s, i)]}
            />
          </svg>
          <span>{s.name}</span>
          {point && point[s.key] !== undefined && (
            <span className="tabular-nums text-foreground">
              {fmtNum(Number(point[s.key]))}
              {unit ? ` ${unit}` : ''}
            </span>
          )}
        </li>
      ...
      {compare && (
        <li className="flex items-center gap-1.5">
          ...
          {point && point[PREV] !== undefined && (
            <span className="tabular-nums text-foreground">
              {fmtNum(Number(point[PREV]))}
              {unit ? ` ${unit}` : ''}
            </span>
          )}
...
export function TimeChart({
  ...
  const delta = useMemo(() => {
    ...
    const then = compare.data.reduce(
      (a, p) => a + (Number(p[primaryKey]) || 0),
      0
    )
  ...
  const anomalyAt = useMemo(
    () => new Map(anomalies.map((a) => [a.x, a])),
    [anomalies]
  )
  ...
  const markerAt = useMemo(
    () => new Map(markers.map((m) => [m.x, m.id])),
    [markers]
  )
  const open = clusters.find(
    (c) => c.key === openCluster && c.members.length > 1
  )
  ...
  if (import.meta.env.DEV && series.length > 4)
    console.warn(
      `[chart] TimeChart has ${series.length} series; more than four lines cannot be told apart by pattern alone. Use small multiples or a table (handoff §8, data-viz.md).`
    )
  return (
    <div className={cn('space-y-1', className)}>
          ...
          <table className="w-full font-mono text-[11px]">
            ...
            <thead>
              <tr>
                <th
                  scope="col"
                  className="op-label sticky top-0 z-10 border-b bg-background px-2 py-1 text-left text-[9px]"
                >
                  bucket
                </th>
                {series
                  .filter((s) => s.inTable !== false)
                  .map((s) => (
                    <th
                      key={s.key}
                      scope="col"
                      className="op-label sticky top-0 z-10 border-b bg-background px-2 py-1 text-right text-[9px]"
                    >
                      {s.name}
                      {unit ? ` (${unit})` : ''}
                    </th>
                ...
                <tr key={p.t}>
                  ...
                  {series
                    ...
                    .map((s) => (
                      ...
                      >
                        {p[s.key] === undefined || p[s.key] === null
                          ? '—'
                          : fmtNum(Number(p[s.key]))}
                        {/* The × the plot draws, in the row it belongs to: an anomaly a keyboard can reach. */}
                        {s.key === primaryKey && anomalyAt.has(p.t) && (
                          <span
                            className="ml-1.5 text-destructive"
                            title={anomalyAt.get(p.t)?.note}
                          >
                            × out of band
                          </span>
                        )}
                  ...
                  {compare && (
                    <td className="px-2 py-1 text-right tabular-nums text-muted-foreground">
                      {p[PREV] === undefined || p[PREV] === null
                        ? '—'
                        : fmtNum(Number(p[PREV]))}
                    </td>
                  )}
                  {band && (
                    <td className="px-2 py-1 text-right tabular-nums text-muted-foreground">
                      {p[band.lower] === undefined
                        ? '—'
                        : `${fmtNum(Number(p[band.lower]))}–${fmtNum(Number(p[band.upper]))}`}
                    </td>
                  )}
                  ...
                  {band && (
                    <td
                      className={cn(
                        ...
                        outside(p, band, primaryKey)
                          ? 'text-destructive'
                          : 'text-muted-foreground'
                      )}
                    >
                      {vsExpected(p, band, primaryKey)}
                    </td>
                  )}
                </tr>
              ))}
      ...
      {!asTable && (
        <div
          ...
          onMouseLeave={() => {
            setReadout(null)
            if (drag) {
              const r = ordered(drag.from, drag.to)
              setDrag(null)
              if (r.from !== r.to) setSelection(r)
            }
          }}
        >
          <ResponsiveContainer>
            <ComposedChart
              ...
              onMouseDown={(s) => {
                const x = (s as { activeLabel?: unknown })?.activeLabel
                if (selectable && typeof x === 'string')
                  setDrag({ from: x, to: x })
              }}
              onMouseUp={() => {
                if (!drag) return
                const r = ordered(drag.from, drag.to)
                setDrag(null)
                setSelection(r.from === r.to ? null : r)
              }}
              onMouseMove={(s) => {
                const idx = Number(
                  (s as { activeIndex?: unknown })?.activeIndex
                )
                if (!Number.isNaN(idx) && data[idx]) {
                  setReadout(data[idx])
                  if (drag) setDrag((d) => (d ? { ...d, to: data[idx].t } : d))
                }
              }}
            >
              <XAxis
                dataKey="t"
                interval={
                  xInterval ?? Math.max(1, Math.floor(data.length / 4) - 1)
                }
                tickLine={false}
                axisLine={{ stroke: 'var(--op-rule-soft)' }}
                tick={TICK}
              />
              <YAxis
                width={34}
              ...
              {clusters.flatMap((c) =>
                c.members.map((m, j) => {
                  ...
                  const label = many
                    ? `${c.members.length} deploys ${openCluster === c.key ? '▴' : '▾'}`
                    : m.id
                  return (
                    <ReferenceLine
                      key={m.id}
                          ...
                          ? {
                              ...
                              onClick: () =>
                                many
                                  ...
                                  : onHot?.(m.id),
                              style: {
                                cursor: many || onHot ? 'pointer' : 'default',
                              },
                            }
                          : undefined
                      }
              ...
              {band && (
                <Area
                  ...
                  name={band.label ?? 'expected range'}
                  fill="url(#op-hatch-soft)"
                  fillOpacity={1}
                  stroke="var(--op-rule-soft)"
                  strokeWidth={1}
              ...
              <RechartsTooltip
                content={<InkTooltip />}
                cursor={{ stroke: 'var(--op-rule-soft)' }}
                isAnimationActive={false}
              />
              ...
              {series
                .map((s, i) => ({ s, i }))
                .sort(
                  (a, b) => (a.s.top ? 1 : 0) - (b.s.top ? 1 : 0) || b.i - a.i
                )
                .map(({ s, i }) => (
                  <Line
                    key={s.key}
      ...
      {selection && !drag && (
        <div className="flex flex-wrap items-center gap-x-3 border px-2 py-1 font-mono text-[11px]">
          ...
          <span className="text-muted-foreground">
            {selCount} of {data.length} points
          </span>
          <span className="ml-auto text-muted-foreground">
            {selCount} points in the window
          </span>
          <button
            type="button"
          ...
          >
            clear <kbd className="ml-0.5 border px-1 text-[9px]">esc</kbd>
          </button>
        </div>
      )}
      {!selection && !drag && !asTable && selectable && !open && (
        <p className="font-mono text-[10px] text-muted-foreground [@media(pointer:coarse)]:hidden">
          drag across the plot to select a window
        </p>
      )}
      {open && (
        <div className="op-rows border font-mono text-[11px]">
          <div className="flex items-center gap-2 px-2 py-1 text-muted-foreground">
            <span>
              {open.members.length} deploys between{' '}
              {open.members[0].at ?? open.members[0].x} and{' '}
              {open.members[open.members.length - 1].at ??
                open.members[open.members.length - 1].x}
            </span>
            <button
              type="button"
          ...
          {open.members.map((m) => (
            <button
              ...
              onClick={() => onOpen?.(m.id)}
              className={cn(
                'flex w-full items-center gap-3 px-2 py-1 text-left hover:bg-muted',
                hot === m.id && 'bg-muted',
    ...
    <div ref={wrap} className={cn('relative max-w-full', className)}>
        ...
        after={
          custom && (
            <button
              ...
              onClick={() => setOpen((o) => !o)}
              className={cn(
                'h-7 shrink-0 border-l px-2 font-mono',
                isCustom ? 'bg-foreground text-background' : 'hover:bg-muted'
              )}
            >
              {isCustom && custom.from && custom.to
                ? `${fmtWindow(custom.from)} → ${fmtWindow(custom.to)}`
                : 'custom'}
            </button>
          )
        }
      ...
      {custom && open && (
        <form
          ...
          className="absolute right-0 top-full z-30 mt-1 grid w-[min(22rem,calc(100vw-2rem))] gap-2 border bg-background p-3 text-xs shadow-[3px_3px_0_0_var(--foreground)]"
          onSubmit={(e) => {
            e.preventDefault()
            if (draft.from && draft.to && draft.from < draft.to) {
              custom.onChange(draft.from, draft.to)
              onChange('custom')
              setOpen(false)
            }
          }}
        >
          ...
          <label className="grid gap-1">
            <span className="op-label">from</span>
            <input
              type="datetime-local"
              required
              value={draft.from}
          ...
          <label className="grid gap-1">
            <span className="op-label">to</span>
            <input
              type="datetime-local"
              required
              value={draft.to}
          ...
          <p className="font-mono text-[10px] text-muted-foreground">
            {draft.from && draft.to && draft.from >= draft.to
              ...
              : `retention ${retentionLabel} · times are ${custom.zone}`}
          </p>
          <div className="flex justify-end gap-2">
            ...
            <button type="submit" className="op-primary h-7 border px-3">
              apply
            </button>
          </div>
        </form>
