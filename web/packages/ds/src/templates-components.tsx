    ...
    <div className={cn('min-w-0 pt-5', className)}>
      ...
      {trail.length > 0 && (
        <nav
          ...
          className="mb-1 flex min-w-0 items-center gap-1.5 text-xs text-muted-foreground"
        >
          {trail.map((c, i) => (
      ...
      <div className="flex min-w-0 flex-wrap items-baseline gap-x-2 gap-y-0.5">
        <h1 className="op-title min-w-0 truncate">
          {mark && (
            <span
              aria-hidden
              className="mr-2 inline-flex translate-y-[-0.1em] align-middle [&>*]:shrink-0"
            >
              {mark}
            </span>
          )}
...
export function SectionTitle({
  ...
  return (
    ...
    >
      <h2 className="min-w-0 truncate text-sm font-semibold leading-6 tracking-[-0.01em]">
        {title}
      </h2>
      {meta && (
...
export function Lede({
  ...
  if (import.meta.env.DEV && (!facts || facts.length < 3))
    console.warn(
      `[record recipe] Lede "${String(word)}" has ${facts?.length ?? 0} facts; a lede carries the four to six facts the reader wants without scrolling (handoff §7, record page). Put them in \`facts\`, not in the aside.`
    )
  return (
    <div className={cn('op-raise min-w-0 border bg-background', className)}>
      <p className="flex min-w-0 flex-wrap items-baseline gap-x-3 gap-y-1 px-4 py-3">
        <span className="inline-flex items-center gap-2 text-lg font-semibold leading-7 tracking-[-0.01em]">
          <span aria-hidden className={cn('text-base', GLYPH_CLASS[state])}>
            {GLYPH[state]}
          </span>
          {word}
        </span>
...
export function Columns({
  ...
  return (
    <div
      className={cn(
        'op-halves grid gap-x-10 xl:grid-cols-[minmax(0,1fr)_18rem]',
        className
      )}
    >
      {children}
...
export function KeyValue({
  ...
  rows: KV[]
  /** Key over value, 11px: for the narrow aside of a record page. */ compact?: boolean
  className?: string
}) {
  return (
    <dl className={cn('op-kv', compact ? 'text-[11px]' : 'text-xs', className)}>
      {rows.map((r) => (
        <div
          key={r.k}
          className={cn(
            'grid grid-cols-1 gap-x-4 gap-y-0.5 px-3 py-2',
            !compact && 'sm:grid-cols-[11rem_minmax(0,1fr)]'
          )}
        >
      ...
      {items.map((e, i) => {
        const tone =
          e.state === 'error'
            ...
            : e.state === 'warn'
              ? 'text-warning'
              : e.state === 'idle' || e.state === 'sampled'
                ? 'text-muted-foreground'
                : 'text-foreground'
        return (
          <li
            key={i}
            className="relative grid grid-cols-[1.5rem_minmax(0,1fr)_auto] gap-x-3 px-3 py-2.5"
          >
            {i < items.length - 1 && (
              <span
                aria-hidden
                className="absolute bottom-0 left-[1.5rem] top-[1.9rem] w-px bg-[var(--op-rule-soft)]"
              />
            )}
            <span
              aria-hidden
              className={cn(
                'relative z-10 flex h-6 w-6 items-center justify-center border bg-background [&_svg]:h-3.5 [&_svg]:w-3.5',
                tone,
                !e.icon && 'text-[10px]'
              )}
            >
...
function ScrollRow({
  ...
  return (
    <div className={cn('relative min-w-0 max-w-full', className)}>
      <div ref={ref} role={role} className="op-scroll-x flex">
        {children}
      </div>
      {edge.l && (
        <span
          aria-hidden
          className="pointer-events-none absolute inset-y-px left-px w-6 bg-gradient-to-r from-background to-transparent"
        />
      )}
      {edge.r && (
        <span
          aria-hidden
          className="pointer-events-none absolute inset-y-px right-px w-6 bg-gradient-to-l from-background to-transparent"
        />
      )}
    </div>
  )
...
export function ActionBar({
  ...
  return (
    <ScrollRow className={cn('op-actions w-full sm:w-auto', className)}>
      <div className="flex w-full items-center gap-2 sm:flex-wrap sm:justify-end [&>*]:shrink-0">
        {children}
      </div>
    </ScrollRow>
  )
...
export function Pager({
  ...
  const btn =
    'inline-flex h-7 items-center gap-0.5 px-1.5 hover:bg-muted hover:text-foreground disabled:opacity-40 disabled:hover:bg-transparent'
  return (
    <span
      className={cn(
        'inline-flex flex-wrap items-center gap-x-2 font-mono text-[11px] text-muted-foreground',
        className
      )}
    >
      ...
      {p.onPageSize && (
        <label className="inline-flex items-center gap-1">
          <select
            ...
            className="h-5 border bg-background px-1 font-mono text-[11px] text-foreground"
          >
            {(p.sizes ?? PAGE_SIZES).map((n) => (
...
export function Ledger({
  ...
  useEffect(() => {
    listRef.current
      ...
      .forEach((el) => {
        ...
        } else if (el.title === el.textContent) el.removeAttribute('title')
      })
  }, [sorted, dense])
  const focusRow = (i: number) => {
    setCursor(i)
    listRef.current
      ?.querySelectorAll<HTMLElement>('.op-row[tabindex]')
      [i]?.focus()
  }
  useEffect(() => {
    const onKey = (e: globalThis.KeyboardEvent) => {
      ...
      if (e.key === 'j' || e.key === 'ArrowDown') {
        e.preventDefault()
        focusRow(Math.min(sorted.length - 1, cursor + 1))
      } else if (e.key === 'k' || e.key === 'ArrowUp') {
        e.preventDefault()
        focusRow(Math.max(0, cursor - 1))
      } else if (
        page &&
        e.key === ']' &&
        page.page < Math.ceil(page.total / page.pageSize)
      ) {
        page.onPage(page.page + 1)
        setCursor(0)
      } else if (page && e.key === '[' && page.page > 1) {
        page.onPage(page.page - 1)
        setCursor(0)
      } else if (e.key === 'Enter' && e.target === document.body)
        sorted[cursor]?.onOpen?.()
      else if (filterable && e.key === '/') {
        e.preventDefault()
  ...
  const cols = {
    '--cols': grid.replace(/(?<![\w(,.])(\d*\.?\d+)fr\b/g, 'minmax(6rem,$1fr)'),
  } as CSSProperties
  return (
    <div className="space-y-4">
      {title && <PageTitle title={title} meta={meta} />}
      {status}
      {(filterable || hint || action) && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {hint && (
            <span className="order-last basis-full text-[11px] text-muted-foreground md:order-none md:ml-auto md:basis-auto">
              {hint}
            </span>
          )}
          {action && (
            <ActionBar className={cn(!hint && 'sm:ml-auto')}>
              {action}
            </ActionBar>
          )}
        </div>
      ...
      {state ?? (
        <div
          ref={listRef}
          className="op-rows op-scroll-x op-cols border md:grid"
          style={cols}
        >
            ...
            {cols_.map((h, i) => {
              ...
              return h.key && sortable ? (
                ...
                <button
                  ...
                  className={cn(
                    'op-label group flex min-w-0 items-center gap-1 text-left hover:text-foreground',
                    h.numeric && 'justify-end text-right',
          ...
          {sorted.map((r, i) => (
            <div
              ...
              className={cn(
                'op-row relative grid w-full grid-cols-[1fr_auto] items-center gap-x-3 text-left text-xs outline-none focus-visible:outline-2 focus-visible:-outline-offset-2 focus-visible:outline-ring md:col-span-full md:grid-cols-subgrid',
                r.onOpen ? 'cursor-pointer' : 'cursor-default',
                i === cursor && 'bg-muted',
                dense ? 'py-0' : 'py-1 md:py-0'
              )}
            >
              {i === cursor && (
                <span
              ...
              {r.cells.map((c, j) => (
                <span
                  ...
                  className={cn(
                    ...
                    cols_[j]?.numeric && 'text-right',
                    j === 0 && r.icon && 'md:flex md:items-center md:gap-2'
                  )}
                >
                  {j === 0 && r.icon && (
                    <span
          ...
          {sorted.length === 0 && (
            <div className="op-row flex items-center text-xs text-muted-foreground md:col-span-full">
              {filterable && filter ? (
                <>
                  no match for &quot;{filter}&quot; ·{' '}
                  <button
          ...
          <div className="op-row flex flex-wrap items-center gap-y-1 text-[11px] text-muted-foreground md:col-span-full">
            ...
            {page ? (
              <>
                <Pager page={page} className="mr-2" />
                {footer ? <span className="mr-2">{footer}</span> : null}
                <span className="hidden lg:inline">
                  · <Kbd keys="[" className="mx-1" />
                  <Kbd keys="]" className="mr-1" /> page ·{' '}
                  <KbdPair
                    keys={['j', 'k']}
                    does={['down', 'up']}
                    className="mx-1"
                  />{' '}
                  · <Kbd keys="⏎" className="mx-1" /> open
                  {filterable && (
                    <>
                      {' '}
            ...
            {sort && (
              <span className="ml-auto flex items-center gap-1">
                sorted by{' '}
                {cols_.find((c) => c.key === sort.key)?.label ?? sort.key}{' '}
                {sort.dir === 'asc' ? '▲' : '▼'} ·{' '}
                <button
                  type="button"
...
function revealInRow(el: HTMLElement) {
  ...
  if (left < row.scrollLeft) row.scrollTo({ left: Math.max(0, left - 8) })
    ...
    row.scrollTo({ left: right - row.clientWidth + 8 })
}

export function Detail<T extends string>({
  title,
  meta,
  mark,
  status,
  ...
  if (import.meta.env.DEV) {
    if (lede) {
      if (!meta)
        console.warn(
          '[record recipe] Detail with a lede has no meta; the meta places the record (id · project · environment) so the aside does not have to.'
        )
      if (!status)
        console.warn(
          '[record recipe] Detail with a lede has no status; every record has a verdict, and "nothing to do" is a verdict.'
        )
    } else if (
      status &&
      meta &&
      Children.toArray(children).some(
        (c) => isValidElement(c) && c.type === Columns
      )
    ) {
      ...
      console.warn(
        `[record recipe] Detail "${String(title ?? '')}" is a record page (status + meta + Columns) with no lede; the lede is the one raised block and the first thing the eye lands on.`
      )
    }
  }
  useEffect(() => {
    ...
    <div className="space-y-4">
      {lede ? (
        <div className="flex min-w-0 flex-wrap items-end justify-between gap-x-6 gap-y-2">
          ...
          {actions && !tabs && (
            <ActionBar className="pt-5">{actions}</ActionBar>
          )}
        </div>
      ) : (
      ...
      {(tabs || (actions && !lede)) && (
        <div className="flex flex-wrap items-center gap-2">
          ...
          {tabs && (
            <ScrollRow
              role="tablist"
              className="[&>div]:border [&>div]:text-xs"
            >
              {tabs.map((t, i) => (
                <button
                  key={t}
                ...
                >
                  ...
                  <Kbd
                    keys={String(i + 1)}
                    className="ml-1 hidden opacity-60 sm:inline-flex"
                  />
                </button>
              ))}
            </ScrollRow>
...
export function Segmented<T extends string>({
  ...
  return (
    <div
      className={cn(
        'op-scroll-x flex min-w-0 max-w-full border text-xs',
        className
      )}
    >
      {options.map(([v, l], i) => (
    ...
    <div className="@container space-y-4">
      ...
      <div className="grid gap-6 @3xl:grid-cols-[180px_minmax(0,1fr)]">
        <nav className="hidden text-xs @3xl:block">
          ...
          <a href="#s-danger" className="block py-1 text-destructive">
            danger zone
          </a>
        </nav>
        <div className="space-y-6">
          ...
          <section id="s-danger" className="border border-destructive">
            <div className="border-b border-destructive px-4 py-2">
              <h2 className="op-h3 text-destructive">danger zone</h2>
            </div>
      ...
      <div
        className={cn(
          'op-sticky-bottom flex items-center gap-3 border-t bg-background px-4 py-2 text-xs @3xl:-mx-6 @3xl:px-6',
          !dirty && !saving && 'text-muted-foreground'
        )}
      >
        <span>
        ...
        <Button
          ...
          busyLabel={
            <>
              <Loader2 aria-hidden /> saving…
            </>
          }
          onClick={onSave}
          className={cn(
            'op-primary ml-auto h-8 text-xs',
            pressed && 'op-pressed'
          )}
        >
...
export function Field({
  ...
  const describedBy =
    [advice ? hintId : null, error ? errId : null].filter(Boolean).join(' ') ||
    undefined
  ...
  const rest = (
    <span className="space-y-1">
      {body}
      {advice && (
        <span id={hintId} className="block text-[11px] text-muted-foreground">
          {advice}
        </span>
      )}
      {error && (
        <span
          id={errId}
          className="flex items-baseline gap-1.5 text-[11px] text-destructive"
        >
  ...
  const cls =
    'grid gap-1 text-xs @md:grid-cols-[160px_minmax(0,1fr)] @md:items-center @md:gap-4'
  if (wired)
    return (
      <div className={cls}>
        <label htmlFor={fid} className="op-label">
          {name}
        </label>
        {rest}
      </div>
