...
export function UsageBar({
  ...
  format = (n) => fmtNum(n),
  sampledFrom,
  sampledLabel = 'sampled past here',
  resets,
  warnAt = 80,
  action,
  className,
  ...
  const word =
    over > 0
      ? `${format(over)}${unit ? ` ${unit}` : ''} over`
      : `${format(Math.max(0, allowance - used))}${unit ? ` ${unit}` : ''} left`
    ...
    <div className={cn('min-w-0 border bg-background p-3 text-xs', className)}>
      ...
      <p className="flex flex-wrap items-baseline gap-x-2">
        ...
        <span
          className={cn(
            ...
            state === 'ok' ? 'text-muted-foreground' : GLYPH_CLASS[state]
          )}
        >
          {state !== 'ok' && state !== 'idle' && (
      ...
      {allowance > 0 && (
        <div
          role="img"
          aria-label={sentence}
          className="relative mt-2 h-4 border"
        >
          <span
            aria-hidden
            className="absolute inset-y-0 left-0 bg-foreground"
            style={{
              width: `${Math.min(100, pctOf(Math.min(used, allowance)))}%`,
              opacity: 0.8,
            }}
          />
          ...
          {over > 0 && (
            <span
              aria-hidden
              className="op-ink-hatch-error absolute inset-y-0 border-l border-destructive"
      ...
      <p className="mt-1.5 flex flex-wrap items-center gap-x-3 font-mono text-[10px] text-muted-foreground">
        ...
        {sampledFrom !== undefined && sampledFrom < used && (
          <span>
            · ◌ {sampledLabel}, from {format(sampledFrom)}
            {unit ? ` ${unit}` : ''} · the figure past it is an estimate
          </span>
        )}
        {over > 0 && (
          <span className="text-destructive">
            · ▨ {format(over)}
            {unit ? ` ${unit}` : ''} over the allowance
          </span>
        )}
...
export function Gauge({
  ...
  const pct = (n: number) =>
    Math.max(0, Math.min(100, (n / Math.max(1e-9, max)) * 100))
  ...
  return (
    <div className={cn('min-w-0 p-3', className)}>
      ...
      <p
        className={cn(
          'mt-1 flex items-baseline gap-2 text-lg leading-6',
      ...
      >
        ...
        {crossed && !idle && (
          <span
            className={cn('font-mono text-[11px]', GLYPH_CLASS[crossed.state])}
          >
            <span aria-hidden>{GLYPH[crossed.state]}</span> {crossed.label}
          </span>
        )}
      </p>
      <div
        role="img"
        aria-label={idle ? `${label}: no samples` : sentence}
        className="relative mt-1.5 h-2 border"
      >
        {!idle && (
          <span
            aria-hidden
            className="absolute inset-y-0 left-0 bg-foreground"
            style={{ width: `${pct(value)}%`, opacity: 0.8 }}
          />
        )}
        {thresholds.map((t) => (
          <span
            ...
            className={cn(
              ...
              t.state === 'error' ? 'bg-destructive' : 'bg-warning'
            )}
            style={{ left: `${pct(t.at)}%` }}
          />
        ))}
      ...
      {!idle && thresholds.length > 0 && (
        <p className="font-mono text-[10px] text-muted-foreground">
          {thresholds
            .map((t) => `┆ ${t.label} ${fmtNum(t.at)}${unit}`)
            .join(' · ')}
        </p>
      )}
    </div>
