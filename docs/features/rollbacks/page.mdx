...
# Rollback to a specific deployment, skipping the deployment picker
# (still asks for a yes/no confirmation before proceeding)
...
There's no separate, configurable "automatic rollback" feature to set up — but as long as a health-check path is configured, you get equivalent protection for free from how deploys work. A new deployment's containers must pass that health check before Temps switches routing to them; if the check fails, the deployment is marked failed and cleaned up, and whatever was previously running (if anything) simply keeps serving traffic the whole time — routing was never switched away from it. See [How rollbacks work](#how-rollbacks-work) above for the full process. If no health-check path is configured for the project, this check is skipped entirely and a broken container can go live — configure a health-check path so this protection actually applies.

This means a manual rollback is only something you need for a deployment that passed its health checks (or has none configured) but is still broken in some way health checks don't catch (a bad business-logic bug, a misconfigured integration, and so on).
...
# 789   production    deployed   main     a1b2c3d   2024-01-15 10:30
# 456   production    deployed   main     e4f5g6h   2024-01-15 09:15
# 123   production    deployed   main     i7j8k9l   2024-01-15 08:00
```
