...
<Note>
Temps includes unlimited scheduled tasks on every project and environment at no additional cost.
</Note>
...
No configuration is required. The secret is generated per project/environment and stays stable across redeployments.
...
- **Isolated execution** - Production and staging crons do not interfere with each other
...
<Properties>
  ...
  <Property name="401 Unauthorized errors">
    ...
    **Solution:**
    - `CRON_SECRET` is automatically injected by Temps on every deployment. You do not need to set it manually.
