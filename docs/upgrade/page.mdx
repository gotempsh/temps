...
While Temps handles migrations automatically, it is good practice to back up before upgrading:
...
Database migrations are **idempotent** — re-running a version that has already been migrated is safe and has no effect.
