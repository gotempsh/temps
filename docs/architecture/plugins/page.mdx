...
Every tagged release publishes per-platform binaries on [GitHub Releases](https://github.com/gotempsh/plugins/releases) — drop one into `~/.temps/plugins/`, click **Reload Plugins** in the dashboard, and it is live. Or clone the repo and run `cargo build --release` to build from source.
