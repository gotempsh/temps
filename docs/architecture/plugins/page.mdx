...
export const sections = [
  ...
  { title: 'TypeScript Plugins', id: 'typescript' },
...
> **Looking for working code?** Official Rust plugins — Lighthouse audits, IndexNow, Google Indexing, and a minimal hello-world — live in [`gotempsh/plugins`](https://github.com/gotempsh/plugins). The TypeScript SDK and its compiled example live in the main Temps repository under [`sdks/node`](https://github.com/gotempsh/temps/tree/main/sdks/node).
...
The [`gotempsh/plugins`](https://github.com/gotempsh/plugins) repository ships four Rust reference plugins that demonstrate the external plugin SDK end-to-end. Each one is a standalone Cargo crate with source, tests, and an optional Vite/React UI bundle. The main repository also contains TypeScript/Bun examples that compile to the same standalone plugin format.
...
| [`typescript-hello-plugin`](https://github.com/gotempsh/temps/tree/main/sdks/node/examples/node-hello-plugin) | Complete TypeScript plugin with authenticated routes, platform queries, events, an embedded React UI, and a protocol integration test. |
| [`deployment-pulse-plugin`](https://github.com/gotempsh/temps/tree/main/sdks/node/examples/deployment-pulse-plugin) | Practical, read-only TypeScript dashboard for cross-project deployment health, recent failures, search, and health filters. |
...
## TypeScript Plugins {{ anchor: true, id: 'typescript' }}

Install [`@temps-sdk/plugin`](https://www.npmjs.com/package/@temps-sdk/plugin) and use Bun to compile a TypeScript entry point into a standalone executable. Bun is a build dependency only; the Temps host does not need a JavaScript runtime.

```bash
bun add @temps-sdk/plugin
bun build src/index.ts --compile --outfile dist/temps-my-plugin
```

```ts
import { createManifest, extractAuthContext, runPlugin, type TempsPlugin } from '@temps-sdk/plugin'

const plugin: TempsPlugin = {
  manifest: () =>
    createManifest('my-plugin', '0.1.0')
      .displayName('My Plugin')
      .description('A plugin built with TypeScript')
      .build(),
...
The SDK implements external-plugin protocol v2, verifies the host's per-process request assertion before exposing caller identity, and provides typed read-only platform queries. Plugins that need the platform HTTP API must declare `api_read` or `api_write` and use `context.apiAsCaller(caller)` so Temps re-applies the caller's permissions.

Start with the [complete TypeScript example](https://github.com/gotempsh/temps/tree/main/sdks/node/examples/node-hello-plugin), which includes build scripts and an executable-level integration test. Then inspect [Deployment Pulse](https://github.com/gotempsh/temps/tree/main/sdks/node/examples/deployment-pulse-plugin) for a production-shaped, read-only dashboard built entirely on typed SDK calls.
