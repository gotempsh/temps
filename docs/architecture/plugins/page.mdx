...
The [`gotempsh/plugins`](https://github.com/gotempsh/plugins) repository ships four Rust reference plugins that demonstrate the external plugin SDK end-to-end. Each one is a standalone Cargo crate with source, tests, and an optional Vite/React UI bundle. The main repository also contains a TypeScript/Bun example that compiles to the same standalone plugin format.
...
Start with the [complete TypeScript example](https://github.com/gotempsh/temps/tree/main/sdks/node/examples/node-hello-plugin), which includes build scripts and an executable-level integration test.
