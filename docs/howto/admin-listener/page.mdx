...
- You expose the Temps control plane on a public IP and do not want random scanners hitting `/api/auth/login`.
...
If you are running Temps on a private network with no public exposure, the default single-listener mode is fine — leave `TEMPS_CONSOLE_ADMIN_ADDRESS` unset.
...
- `configure_public_routes` — the **public** surface. SDK ingest endpoints, webhook delivery targets, worker-facing APIs. These handlers authenticate themselves (API key, DSN token, Host-header lookup) — they do not rely on session auth.

When `TEMPS_CONSOLE_ADMIN_ADDRESS` is set, the server binds both routers on separate `axum::serve` listeners. When it is unset, both routers are merged onto `TEMPS_CONSOLE_ADDRESS` for backwards compatibility.
...
<Note>
  **Webhooks are public on purpose.** GitHub, Stripe, and AWS SES must reach
  ...
  signature, so the public listener does not weaken the security model.
</Note>
...
- **Credential-stuffing attacks** that do not know the admin surface exists.
