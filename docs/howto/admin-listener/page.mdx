...
Everything else — `/api/auth/*`, `/api/projects/...`, settings, deployments, alert rules, error-group queries, source-map management, the AI gateway, AI gateway usage analytics, the dashboard SPA, Swagger UI at `/swagger-ui`.
...
<Note>
  **The AI gateway is _not_ public ingest.** The OpenAI-compatible endpoints
  (`POST /api/ai/v1/chat/completions`, `POST /api/ai/v1/embeddings`,
  `GET /api/ai/v1/models`) register only via `configure_routes`, so they live
  on the **admin** surface and require an authenticated API key. If you split
  listeners, deployed apps calling the AI gateway must reach the admin
  address — don't leave it on the open public listener.
</Note>
