...
- `validate` - Validate a Temps-native template YAML file offline
...
Validate a Temps-native template YAML file offline
...
Show which capabilities the server process actually provides: its serve
profile (`full` or `control-plane`), whether it deploys containers locally,
runs managed services, takes local backups, runs agent sandboxes, and whether
a Docker daemon answered at startup. A control plane started with
`temps serve --profile control-plane` reports `false` for the local
capabilities — applications run on worker nodes joined with `temps join`.
...
- `enrich <visitor-id>` - Attach identity or attributes to a visitor
...
### `analytics enrich <visitor-id>`

Attach identity or attributes to a visitor. The top-level keys you send are merged into the visitor's stored `custom_data`; a key sent with a `null` value is removed. The visitor ID is a numeric ID, a visitor GUID, or the sealed `enc_...` ID from the `_temps_visitor_id` cookie.

**Options:**

| Flag                | Description                                                                        | Default | Required |
| ------------------- | ---------------------------------------------------------------------------------- | ------- | -------- |
| `-d, --data <json>` | JSON object to merge                                                               | -       | No       |
| `-f, --file <path>` | Read the JSON object to merge from a file                                          | -       | No       |
| `--set <key=value>` | Set one key to a string value (repeatable)                                         | -       | No       |
| `--unset <key>`     | Remove one top-level key (repeatable; sends null)                                  | -       | No       |
| `--json`            | Output in JSON format                                                              | -       | No       |

At least one of `--data`, `--file`, `--set`, or `--unset` is required. Sources merge lowest to highest: `--file`, `--data`, `--set`, `--unset`. A visitor that does not exist is reported as a failure with a non-zero exit code.

**Examples:**

```bash
bunx @temps-sdk/cli analytics enrich enc_AbC123 --data '{"user_id":"user_123","email":"ada@example.com"}'
bunx @temps-sdk/cli analytics enrich 4821 --set plan=pro --set segment=enterprise
bunx @temps-sdk/cli analytics enrich 4821 --file ./visitor.json
bunx @temps-sdk/cli analytics enrich enc_AbC123 --unset trial_ends_at --json
```
