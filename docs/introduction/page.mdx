...
Once complete, open the console URL shown at the end and you are ready to deploy.
...
<Note>
  If you do not have Docker installed, run: `curl -fsSL https://get.docker.com |
  sh`
</Note>
...
Wait a few seconds for the database to initialize, then verify it is running:
...
If Temps hangs during startup, it is likely waiting for the screenshot provider to initialize. Disable it with:
