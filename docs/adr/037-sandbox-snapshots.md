...
2. `exec_as_root` to clear any injected env vars from the running container's environment. Docker does not expose a live env-edit API, but the commit can be post-processed: `docker inspect` reads the committed image's `Config.Env`, the scrubber computes the diff between the current env and the base image's known env, removes all entries that were injected (not present in the base), and re-tags the image with a `docker build --no-cache` `FROM scratch`-style rewrite that only carries the scrubbed env. In practice, the simpler path for v1 is to pass a list of env keys to scrub to `docker commit --change "ENV KEY="` for each injected key.
3. Verify scrubbing by inspecting the committed image's `Config.Env` and rejecting the snapshot if any known-sensitive key patterns remain (reject, not silently proceed).
...
The sandbox is stopped for the duration of the snapshot operation (for Docker commit consistency) and restarted when the snapshot completes, unless the sandbox was already stopped. The caller sees the sandbox status flip to `stopped` and back to `running` as a normal lifecycle event.
