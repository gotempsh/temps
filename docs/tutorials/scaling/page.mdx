...
Each container can be given CPU and memory limits. New projects start with a conservative default profile rather than being fully uncapped — a scheduling request plus a hard memory limit, so a runaway app can't OOM a small single-node host. You can raise, lower, or uncap these per environment at any time.

| Setting | Default | Description |
|---------|---------|-------------|
| CPU request | 0.5 CPU | Minimum guaranteed CPU |
| CPU limit | Unset (uncapped) | Maximum CPU, if configured |
| Memory request | 128 MB | Minimum guaranteed memory |
