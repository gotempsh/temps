...
For a pull-based install that skips compiling the Rust workspace on the target
host, use `docker-compose.release.yml` instead of `docker-compose.yml` — same
`.env`/secrets setup above, but the `temps` service pulls the published
`ghcr.io/gotempsh/temps` image (`docker compose -f docker-compose.release.yml up --detach`).
...
few seconds. Forwarded-IP trust is off by default.
