...
- **Template env var generators**: `EnvVarTemplate` gains a `default_generator` field with three client-side generators — `app_url` (https://{repo}.{preview_domain}), `random_hex_32`, and `random_secret`. The `app_url` generator reads the platform's configured `preview_domain` (with `external_url` host as fallback) so generated URLs match the proxy's actual routing rules instead of guessing `temps.sh`. Auto-fills required vars on mount, recomputes `app_url`-style values when the repository name or platform settings change (only if the user hasn't manually edited the field), and adds a per-field "Generate" button (Sparkles icon). Wired `NEXTAUTH_URL → app_url` and `NEXTAUTH_SECRET → random_hex_32` in the bundled Next.js SaaS Starter
