...
### Fixed
- **`/projects/new` browser navigation**: the source picker (Import / Template / Git URL / Manual) now mirrors the chosen mode to a `?source=` query param, so browser back/forward, deep links, and tab restore all work. Picking a source pushes a new history entry; pressing back returns to the picker. Local state for stale sub-screens (e.g. a selected template) is cleared automatically when the URL source changes. The inline mode (used inside the onboarding flow) continues to use plain React state since it doesn't own the URL.
