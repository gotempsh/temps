...
- **PostgreSQL data-explorer query isolation**: Reject function calls and every PostgreSQL subquery form in user-supplied WHERE clauses so expressions cannot read other tables through XML helpers or `IN (TABLE ...)` query expressions.
