...
use git2::{Repository, Signature, BranchType, PushOptions, RemoteCallbacks, Cred, CredentialType};
use tracing::debug;
...
impl GitService {
    ...
    pub async fn get_status(&self, repo_path: &Path) -> Result<GitStatus> {
        ...
        task::spawn_blocking(move || {
            ...
            for status in statuses.iter() {
                ...
                if flags.is_index_new() || flags.is_index_modified() || flags.is_index_deleted() {
                    staged.push(GitFileStatus {
                        path: path.clone(),
                        status: if flags.is_index_new() { "A".to_string() }
                               else if flags.is_index_modified() { "M".to_string() }
                               else if flags.is_index_deleted() { "D".to_string() }
                               else { "?".to_string() }
                    });
                }
                ...
                if flags.is_wt_modified() || flags.is_wt_deleted() {
                    modified.push(GitFileStatus {
                        path: path.clone(),
                        status: if flags.is_wt_modified() { "M".to_string() }
                               else { "D".to_string() }
                    });
                }
            ...
            let has_uncommitted_changes = !staged.is_empty() || !modified.is_empty() || !untracked.is_empty();
    ...
    pub async fn unstage_files(&self, repo_path: &Path, files: Option<Vec<String>>) -> Result<String> {
        let repo_path = repo_path.to_path_buf();
        ...
        task::spawn_blocking(move || {
            ...
            if let Some(file_list) = files {
                if file_list.is_empty() {
                    ...
                    repo.reset_default(Some(&head_commit.as_object()), file_list.iter().map(|s| s.as_str()))?;
                }
            } else {
                // Reset all files
                repo.reset_default(Some(&head_commit.as_object()), ["*"].iter())?;
    ...
    pub async fn remove_files(&self, repo_path: &Path, files: Vec<String>, delete_from_disk: bool) -> Result<String> {
        let repo_path = repo_path.to_path_buf();
    ...
    /// Commit changes
    pub async fn commit(&self, repo_path: &Path, message: &str, author_name: &str, author_email: &str) -> Result<String> {
        let repo_path = repo_path.to_path_buf();
    ...
    pub async fn pull(&self, repo_path: &Path, branch: &str, credentials: GitCredentials) -> Result<String> {
        let repo_path = repo_path.to_path_buf();
        ...
        task::spawn_blocking(move || {
            ...
            cb.credentials(|_url, _username_from_url, allowed_types| {
                if allowed_types.contains(CredentialType::USER_PASS_PLAINTEXT) {
                    Cred::userpass_plaintext(&credentials.username, &credentials.personal_access_token)
                } else {
                    Cred::default()
                }
            });
            ...
            let _fetch_head = repo.fetchhead_foreach(|ref_name, remote_url, oid, _is_merge| {
                debug!("Fetched {}: {} from {}", ref_name, oid, String::from_utf8_lossy(remote_url));
                true
            })?;
    ...
    pub async fn push(&self, repo_path: &Path, branch: &str, credentials: GitCredentials) -> Result<String> {
        let repo_path = repo_path.to_path_buf();
        let branch = branch.to_string();
    ...
    /// Create a new branch
    pub async fn create_branch(&self, repo_path: &Path, branch_name: &str, from_branch: Option<&str>) -> Result<String> {
        let repo_path = repo_path.to_path_buf();
        ...
        task::spawn_blocking(move || {
            ...
            let branch_ref = branch.get().name().ok_or_else(|| anyhow!("Invalid branch reference"))?;
    ...
    fn get_ahead_behind_count(repo: &Repository, branch: &str) -> Result<(u32, u32)> {
        ...
        let local_oid = match repo.find_reference(&local_ref) {
            Ok(reference) => reference.target().ok_or_else(|| anyhow!("No target for local branch"))?,
            Err(_) => return Ok((0, 0)),
        };
        
        let remote_oid = match repo.find_reference(&remote_ref) {
            Ok(reference) => reference.target().ok_or_else(|| anyhow!("No target for remote branch"))?,
            Err(_) => return Ok((0, 0)),
        };
