...
use super::git_provider::{
    GitProviderService, GitProviderType, GitProviderError,
    Repository, Branch, Commit, FileContent, WebhookConfig, User, AuthMethod, GitProviderTag
};
...
impl GitLabProvider {
    ...
    async fn refresh_access_token(
        ...
        let token_response: TokenResponse = response
            .json()
            .await
            .map_err(|e| GitProviderError::ApiError(format!("Failed to parse refresh response: {}", e)))?;
...
impl GitProviderService for GitLabProvider {
    ...
    async fn authenticate(&self, code: Option<String>) -> Result<String, GitProviderError> {
        match &self.auth_method {
            AuthMethod::GitLabApp { app_id, app_secret, redirect_uri } => {
                if let Some(code) = code {
                    ...
                    Err(GitProviderError::AuthenticationFailed("Authorization code required".to_string()))
                }
            },
            AuthMethod::OAuth { client_id, client_secret, redirect_uri } => {
                if let Some(code) = code {
                    // Exchange authorization code for access token
                    let client = self.get_client();
                    let params = [
    ...
    async fn get_auth_url(&self, state: &str) -> Result<String, GitProviderError> {
        match &self.auth_method {
            AuthMethod::GitLabApp { app_id, redirect_uri, .. } |
            AuthMethod::OAuth { client_id: app_id, redirect_uri, .. } => {
                let auth_url = format!(
        ...
        match self.validate_token(access_token).await {
            ...
            Ok(false) | Err(GitProviderError::RateLimitExceeded) => {
                ...
                if let Some(refresh_token) = refresh_token {
                    ...
                    match &self.auth_method {
                        AuthMethod::GitLabApp { app_id, app_secret, .. } => {
                            let (new_access_token, new_refresh_token) =
                                self.refresh_access_token(app_id, app_secret, refresh_token).await?;
                            Ok((new_access_token, new_refresh_token))
                        },
                        AuthMethod::OAuth { client_id, client_secret, .. } => {
                            let (new_access_token, new_refresh_token) =
    ...
    async fn list_repositories(
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to list repositories: {}", response.status())));
        }
        ...
        let repositories = projects
            ...
            .map(|p| {
                ...
                Repository {
                    ...
                    stars: p.star_count,
                    forks: p.forks_count,
                    created_at: chrono::DateTime::parse_from_rfc3339(&p.created_at).map(|dt| dt.with_timezone(&chrono::Utc))
    ...
    async fn get_repository(
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to get repository: {}", response.status())));
        }
        ...
        Ok(Repository {
            ...
            created_at: chrono::DateTime::parse_from_rfc3339(&project.created_at).map(|dt| dt.with_timezone(&chrono::Utc))
                .unwrap_or_else(|_| chrono::Utc::now()),
            updated_at: chrono::DateTime::parse_from_rfc3339(&project.last_activity_at).map(|dt| dt.with_timezone(&chrono::Utc))
                .unwrap_or_else(|_| chrono::Utc::now()),
            pushed_at: None,
        })
    }
    ...
    async fn list_branches(
        ...
        let project_path = format!("{}/{}", owner, repo);
        let encoded_path = urlencoding::encode(&project_path);
        let url = format!("{}/api/v4/projects/{}/repository/branches", self.base_url, encoded_path);
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to list branches: {}", response.status())));
        }
    ...
    async fn list_tags(
        ...
        let url = format!("{}/api/v4/projects/{}/repository/tags", self.base_url, encoded_path);
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to list tags: {}", response.status())));
        }
        ...
        let gitlab_tags: Vec<GitLabTag> = response
            .json()
            .await
            .map_err(|e| GitProviderError::ApiError(format!("Failed to parse tags response: {}", e)))?;
    ...
    async fn get_file_content(
        ...
        let mut url = format!("{}/api/v4/projects/{}/repository/files/{}", self.base_url, encoded_path, file_path);
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to get file content: {}", response.status())));
        }
    ...
    async fn create_webhook(
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to create webhook: {}", response.status())));
        }
    ...
    async fn get_user(&self, access_token: &str) -> Result<User, GitProviderError> {
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to get user: {}", response.status())));
        }
    ...
    async fn get_latest_commit(
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to get latest commit: {}", response.status())));
        }
    ...
    async fn delete_webhook(
        ...
        if !response.status().is_success() {
            return Err(GitProviderError::ApiError(format!("Failed to delete webhook: {}", response.status())));
        }
    ...
    async fn clone_repository(
        ...
        if !output.status.success() {
            ...
            return Err(GitProviderError::Other(format!("Git clone failed: {}", stderr)));
        }
    ...
    async fn get_commit(
        ...
        let url = format!("{}/api/v4/projects/{}/repository/commits/{}",
    ...
    async fn download_archive(
        ...
        info!("Downloading GitLab archive for {}/{} at ref {}", owner, repo, ref_spec);
        ...
        while let Some(chunk) = stream.next().await {
            let chunk = chunk.map_err(|e| GitProviderError::ApiError(format!("Failed to read chunk: {}", e)))?;
        ...
        info!("Successfully downloaded GitLab archive to {:?}", target_path);
        Ok(())
    }
}