...
#[derive(Debug, Clone, PartialEq, Eq, thiserror::Error)]
pub enum ComposeCredentialValidationError {
    #[error(transparent)]
    InvalidCompose(#[from] ComposeParseError),
    #[error(
        "Compose file contains a literal credential at {location}; reference an encrypted project environment variable instead"
    )]
    EmbeddedCredential { location: String },
}

/// Reject credentials that would otherwise be stored in a plaintext Compose
/// source revision. Both catalog preflight and editable-source save call this
/// shared policy so a plan reported as ready cannot fail only after creating
/// its project.
pub fn validate_compose_credentials(
    yaml: &str,
) -> Result<(), ComposeCredentialValidationError> {
    let root = parse_compose_document(yaml, "Compose file")?;
    if yaml_contains_private_key(&root) {
        return Err(ComposeCredentialValidationError::EmbeddedCredential {
        ...
        match environment {
            serde_yaml::Value::Sequence(entries) => {
                for entry in entries.iter().filter_map(serde_yaml::Value::as_str) {
                    ...
                    validate_environment_credential(service_name, name.trim(), value)?;
                }
            }
            serde_yaml::Value::Mapping(entries) => {
                for (name, value) in entries {
                    ...
                    validate_environment_credential(service_name, name, &value)?;
                }
            }
            _ => {}
        }
...
fn url_query_name_is_secret(name: &str) -> bool {
    let normalized: String = name
        ...
        .collect();
    if compose_environment_name_is_secret(&normalized) {
        return true;
    }
...
/// credential rather than public configuration or credential metadata.
pub fn compose_environment_name_is_secret(name: &str) -> bool {
    ...
    if name.is_empty()
        ...
        || environment_name_describes_non_secret_metadata(&name)
    {
        return false;
    }
    const MARKERS: &[&str] = &[
...
fn environment_name_describes_non_secret_metadata(name: &str) -> bool {
    name.starts_with("ALLOW_")
        ...
        || name.ends_with("_RATE_LIMIT")
        || name.ends_with("_INTERVAL")
        || name.ends_with("_TIMEOUT")
        || name.ends_with("_TTL")
        || name.contains("RANDOMPASSWORD")
}
...
mod compose_preview_tests {
    ...
    #[test]
    fn credential_validation_accepts_secret_metadata_and_flags() {
        let yaml = r#"
services:
  app:
    image: example/app
    environment:
      ...
      TRIGGER_TOKEN_PATH: /run/secrets/trigger-token
"#;
    ...
    #[test]
    fn credential_validation_accepts_rate_limit_configuration() {
        let yaml = r#"
services:
  app:
    image: example/app
...
"#;
    ...
    #[test]
    fn credential_validation_rejects_non_empty_variable_fallbacks() {
        for value in ["${API_KEY:-literal}", "${API_KEY-literal}"] {
            let yaml = format!(
                "services:\n  app:\n    image: example/app\n    environment:\n      API_KEY: {value}\n"
            );
            assert!(matches!(
                validate_compose_credentials(&yaml),
                Err(ComposeCredentialValidationError::EmbeddedCredential { .. })
            ));
        }
    }

    #[test]
    fn credential_validation_accepts_required_and_empty_variable_expressions() {
        for value in [
            "${API_KEY}",
            "${API_KEY?required}",
            "${API_KEY:?required}",
            ...
            assert_eq!(validate_compose_credentials(&yaml), Ok(()), "{value}");
        }
    }
