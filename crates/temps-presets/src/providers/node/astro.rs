//! Astro preset provider
//!
//! Detects Astro projects and provides build configuration
...
pub struct AstroPresetProvider;

impl PresetProvider for AstroPresetProvider {
    fn preset(&self) -> Preset {
        Preset::Astro
    }

    fn detect(&self, app: &App) -> Option<Confidence> {
        let has_astro = app.has_any_dependency("astro");

        if !has_astro {
            return None;
        }

        let has_config = app.includes_file("astro.config.mjs")
            || app.includes_file("astro.config.js")
            || app.includes_file("astro.config.ts");
    ...
    fn get_build_config(&self, app: &App) -> Option<BuildConfig> {
        ...
        Some(BuildConfig {
            ...
            start_cmd: "npx serve -s dist -l 4321".to_string(),
            output_dir: Some("dist".to_string()),
            port: 4321, // Astro default dev server port
            static_serve: true, // Astro can produce static files
        })
    }

    fn generate_dockerfile(&self, app: &App) -> Result<String> {
        let build_config = self.get_build_config(app).ok_or_else(|| {
            anyhow::anyhow!("Failed to get build config for Astro")
        })?;
...
mod tests {
    ...
    fn test_detect_astro() {
        ...
        files.insert(
            ...
            r#"{"dependencies":{"astro":"4.0.0"}}"#.to_string(),
        );

        let app = create_test_app(files);
        let provider = AstroPresetProvider;
    ...
    #[test]
    fn test_detect_astro_with_config() {
        let mut files = HashMap::new();
        files.insert(
            "package.json".to_string(),
            r#"{"dependencies":{"astro":"^4.0.0"}}"#.to_string(),
        );
        files.insert("astro.config.mjs".to_string(), "export default {}".to_string());
    ...
    fn test_generate_dockerfile() {
        ...
        assert!(dockerfile.contains("EXPOSE 4321"));
