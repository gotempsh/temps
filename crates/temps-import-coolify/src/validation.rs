...
//! Pre-flight validation rules for Coolify imports
...
pub(crate) mod fixtures {
    ...
    pub fn snapshot(image: Option<&str>, git_repository: Option<&str>) -> WorkloadSnapshot {
        let source_metadata = match git_repository {
            Some(r) => serde_json::json!({ "git_repository": r }),
...
pub struct CoolifyValidationRules;

impl CoolifyValidationRules {
...
impl ImportValidationRule for SourcePresentRule {
    ...
    fn validate(&self, snapshot: &WorkloadSnapshot, _plan: &ImportPlan) -> ValidationResult {
        ...
        ValidationResult {
            ...
            remediation: (!passed)
                .then(|| "Check the application's source configuration in Coolify".to_string()),
    ...
    fn validate(&self, _snapshot: &WorkloadSnapshot, plan: &ImportPlan) -> ValidationResult {
        ...
        ValidationResult {
            ...
            message: if passed {
                ...
                format!(
                    "{} database(s) are not publicly reachable: {} — their data needs a manual dump on the Coolify server",
                    unreachable.len(),
                    unreachable.join(", ")
                )
            },
            remediation: (!passed).then(|| {
                "Enable 'public port' on these databases in Coolify before migrating, or plan a docker-exec dump on the source server".to_string()
            }),
...
/// Coolify-generated wildcard-DNS domains are skipped, which is expected —
    ...
    fn validate(&self, _snapshot: &WorkloadSnapshot, plan: &ImportPlan) -> ValidationResult {
        ...
        ValidationResult {
            ...
            message: if skipped.is_empty() {
                "No Coolify-generated domains to skip".to_string()
            } else {
                format!(
                    "{} Coolify-generated domain(s) will be skipped (temps assigns its own): {}",
                    skipped.len(),
                    skipped.join(", ")
                )
            },
