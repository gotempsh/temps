...
async fn execute_plan(
    ...
    {
        let message = format!(
            "Managed service creation is not yet automated for Dokploy imports — create a {} service named '{}' in temps and migrate its data (see the plan's data implications)",
            service_plan.service_type, service_plan.name
        );
    ...
    {
        let message = format!(
            "Domain registration is not yet automated for Dokploy imports — add '{}' to the environment in temps and update its DNS",
            domain_plan.domain
        );
        warnings.push(message.clone());
        step_results.push(StepResult {
