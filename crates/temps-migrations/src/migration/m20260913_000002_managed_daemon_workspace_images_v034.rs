...
pub(super) const UP_SQL: &str = "ALTER TABLE ai_application_workspaces
    ...
    ADD CONSTRAINT ai_application_workspaces_image_check CHECK (image IS NULL OR image IN (
        ...
        'ghcr.io/gotempsh/temps-sandbox-nodejs:0.3.4',
...
impl MigrationTrait for Migration {
    ...
    async fn down(&self, manager: &SchemaManager) -> Result<(), DbErr> {
        // PostgreSQL rejects the downgrade while any workspace selects 0.3.4.
        manager
            .get_connection()
...
mod tests {
    ...
    #[test]
    fn v034_constraint_preserves_history_and_accepts_new_managed_images() {
        for flavor in ["nodejs", "python", "all"] {
            for version in ["0.2.0", "0.3.0", "0.3.1", "0.3.2", "0.3.3", "0.3.4"] {
                let image = format!("ghcr.io/gotempsh/temps-sandbox-{flavor}:{version}");
                assert!(UP_SQL.contains(&image), "missing {image}");
                assert_eq!(DOWN_SQL.contains(&image), version != "0.3.4");
            }
        }
