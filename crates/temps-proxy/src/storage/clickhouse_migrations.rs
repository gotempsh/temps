...
//! `temps-otel/src/storage/clickhouse/migrations.rs`:
//!
//! 1. Ensure the target database exists (`CREATE DATABASE IF NOT EXISTS`).
//! 2. Create the `_temps_ch_proxy_logs_migrations` tracking table.
...
//!
//! The runner is invoked at construction time when
//! `ServerConfig::is_clickhouse_enabled()` is true. CH DDL is not
//! transactional, so partial rollback is not attempted; the first read/write
//! surfaces any error per-call.
//!
//! # Tracking-table name
//!
//! This runner uses `_temps_ch_proxy_logs_migrations` — DISTINCT from the OTel
//! backend's `_temps_ch_otel_migrations` and the metrics backend's
//! `_temps_ch_metrics_migrations`. All three run against the SAME ClickHouse
...
const TRACKING_DDL: &str = r#"CREATE TABLE IF NOT EXISTS _temps_ch_proxy_logs_migrations
(
    name        String,
...
fn validate_database_name(name: &str) -> Result<(), ProxyLogServiceError> {
    if name.is_empty() {
        return Err(ProxyLogServiceError::ClickHouse {
...
/// Apply all pending proxy-log ClickHouse migrations idempotently.
///
...
pub async fn apply_migrations(
    ...
    database_name: &str,
) -> Result<MigrationReport, ProxyLogServiceError> {
    use ::clickhouse::Row;
    ...
    let applied: Vec<String> = client
        .query("SELECT name FROM _temps_ch_proxy_logs_migrations FINAL")
    ...
    for migration in MIGRATIONS {
        if applied.iter().any(|n| n == migration.name) {
            tracing::debug!(
                ...
                "ch-proxy-logs migration already applied — skipping"
            );
        ...
        tracing::info!(
            migration = migration.name,
            "applying ch-proxy-logs migration"
        );
        ...
        client
            .query("INSERT INTO _temps_ch_proxy_logs_migrations (name) VALUES (?)")
            .bind(migration.name)
            .execute()
            .await
            .map_err(|e| ProxyLogServiceError::ClickHouse {
...
mod tests {
    ...
    fn valid_database_names_pass() {
        ...
        assert!(validate_database_name("otel_proxy").is_ok());
    ...
    fn unsafe_database_names_are_rejected() {
        ...
        assert!(validate_database_name("otel;DROP TABLE proxy_logs--").is_err());
    ...
    /// The proxy-logs tracking table name must be distinct from the otel and
    /// metrics runners, since all three share the `otel` CH database.
    #[test]
    fn tracking_table_name_is_distinct() {
        assert!(TRACKING_DDL.contains("_temps_ch_proxy_logs_migrations"));
        assert!(!TRACKING_DDL.contains("_temps_ch_otel_migrations"));
        assert!(!TRACKING_DDL.contains("_temps_ch_metrics_migrations"));
    }
}
