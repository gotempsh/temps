...
fn test_internal_server_error_builder() {
    ...
    assert_eq!(
        error.body.get("type").unwrap().as_str().unwrap(),
        "https://temps.sh/probs/internal-server-error"
    );
    assert_eq!(
        error.body.get("title").unwrap().as_str().unwrap(),
    ...
    assert_eq!(
        error.body.get("error_code").unwrap().as_str().unwrap(),
...
fn test_not_found_builder() {
    let error = not_found().detail("User with ID 123 was not found").build();

    assert_eq!(error.status_code, StatusCode::NOT_FOUND);
    assert_eq!(
        error.body.get("type").unwrap().as_str().unwrap(),
        "https://temps.sh/probs/not-found"
    );
    assert_eq!(
        error.body.get("title").unwrap().as_str().unwrap(),
        "Resource Not Found"
...
fn test_unauthorized_builder() {
    ...
    assert_eq!(
        error.body.get("type").unwrap().as_str().unwrap(),
        "https://temps.sh/probs/unauthorized"
    );
    assert_eq!(
        error.body.get("title").unwrap().as_str().unwrap(),
        "Unauthorized"
...
fn test_forbidden_builder() {
    ...
    assert_eq!(
        error.body.get("type").unwrap().as_str().unwrap(),
        "https://temps.sh/probs/forbidden"
    );
    assert_eq!(
        error.body.get("title").unwrap().as_str().unwrap(),
        "Forbidden"
...
fn test_error_builder_chaining() {
    ...
    assert_eq!(
        error.body.get("custom_field").unwrap().as_str().unwrap(),
        "custom_value"
    );
    assert!(error.body.contains_key("timestamp"));
}
