...
id=$(rpc '{"version":1,"operation":{"type":"start","name":"test-server","program":"node","args":["-e","console.log(\"ready\");setInterval(()=>{},1000)"],"directory":"."}}' | jq -er '.process.id')
...
echo 'PASS: client disconnect, logs, restart, stop, version rejection, graceful shutdown, volume-preserving replacement'
